...
_TIMEOUT_EXEMPT_PREFIXES = (
    ...
    "/api/dyom/setup",  # remote pacman/apt installs
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and _hdr == _ITT and _client_host in ("127.0.0.1", "::1"):
                    ...
                    # X-Digital Garage-Owner, attribute the request to that
                    ...
                    # asked for them).
                    _impersonate = (
                        request.headers.get("X-Digital Garage-Owner") or ""
                    ).strip()
            ...
            if auth_header.startswith("Bearer dg_"):
                raw_token = auth_header[7:]
                # Sanity check: tokens are "dg_" + 43 chars of base64
...
# DYOM (model download/serve/cache, dyom state sync)
from routes.dyom_routes import setup_dyom_routes

app.include_router(setup_dyom_routes())

# Hardware model fitting (dyom "What Fits?" tab)
...
@app.get("/dyom")
async def serve_dyom(request: Request):
    return await serve_index(request)
...
async def startup_event():
    ...
    # `DIGITAL_GARAGE_INPROCESS_POLLERS` from the email pollers.
    _tasks_inprocess = (
        os.environ.get("DIGITAL_GARAGE_INPROCESS_TASKS", "1").strip().lower()
    )
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        await task_scheduler.start()
    else:
        logger.info(
            "In-process task scheduler disabled (DIGITAL_GARAGE_INPROCESS_TASKS=0); "
