...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.digital_garage-highlight').forEach(function (e) { e.classList.remove('digital_garage-highlight'); });
      document.querySelectorAll('.digital_garage-hl-label').forEach(function (e) { e.remove(); });
      var target = document.querySelector(uiData.selector);
      if (target) {
        target.classList.add('digital_garage-highlight');
        target.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
        if (uiData.label) {
          var lbl = document.createElement('div');
          lbl.className = 'digital_garage-hl-label';
      ...
      if (panel === 'documents') {
        ...
        import('./dyom.js').then(function (mod) {
