...
const GROUP_STATE_KEY = 'digital_garage-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const gids = JSON.parse(localStorage.getItem('digital_garage-group-sessions') || '[]');
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('digital_garage-group-sessions', JSON.stringify(gids)); }
    } catch (e) { }
  } catch (e) {
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          // Text delta (digital_garage format)
