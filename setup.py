...
def create_default_admin():
    ...
    try:
        ...
        username = os.getenv("DIGITAL_GARAGE_ADMIN_USER", "admin").strip() or "admin"
        password = os.getenv("DIGITAL_GARAGE_ADMIN_PASSWORD") or __import__("secrets").token_urlsafe(18)
        ...
        print(f"        ** Change it after first login. Set DIGITAL_GARAGE_ADMIN_PASSWORD to choose your own. **")
    except ImportError:
...
def check_deps():
    ...
    if os.name != "nt" and shutil.which("tmux") is None:
        print("\n  [warn] tmux not found")
        print("         DYOM uses tmux for background downloads and model serves.")
