 ...
 *
 * Every picker ends with a "+ Serve a model in DYOM…" sentinel —
 * choosing it opens DYOM → Serve filtered to image models, then
...
export function wireAIModelSelectors({ container, apiBase, openDYOMForImg2img }) {
  // Delegated handler for the "+ Serve a model in DYOM…" sentinel
  ...
  container.addEventListener('change', (e) => {
    ...
    if (!sel) return;
    if (sel.value !== '__serve_dyom__') return;
  ...
  container.addEventListener('focus', (e) => {
    const sel = e.target.closest('select');
    if (sel && sel.value !== '__serve_dyom__') sel._prevServeValue = sel.value;
  }, true);
  ...
  async function loadAIModels(opts = {}) {
    try {
      ...
      for (const ep of endpoints) {
        ...
        // DYOM model as "(offline)" in the editor picker.
      ...
      // Append the "Serve a model in DYOM…" sentinel at the
      ...
      const appendServeSentinel = (sel) => {
        ...
        serveOpt.value = '__serve_dyom__';
        serveOpt.textContent = '+ Serve a model in DYOM…';
      ...
      const fallback = '<option value="">Auto</option><option value="" disabled>──────────</option><option value="__serve_dyom__">+ Serve a model in DYOM…</option>';
  ...
  // so a model served via DYOM mid-edit shows up without having to
