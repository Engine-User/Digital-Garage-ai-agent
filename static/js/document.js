...
async function _resolveComposeSendAccountId() {
  const activeAccountId = window.__digital_garagedysseusActiveEmailAccount || null;
...
const _docOpenKey = (sessionId) => 'digital_garage-doc-open-' + sessionId;
const _docMinimizedKey = (sessionId) => 'digital_garage-doc-minimized-' + sessionId;
  ...
  try {
    const res = await fetch(`${API_BASE}/api/email/draft`, {
      ...
      body: JSON.stringify({
        ...
        account_id: window.__digital_garageActiveEmailAccount || null,
      }),
    });
    const data = await res.json();
    if (data.success) {
...
export function openPanel() {
  ...
  let _fontIdx = parseInt(localStorage.getItem('digital_garage-doc-fontsize') || '0', 10);
  ...
  function _applyDocFont() {
    ...
    localStorage.setItem('digital_garage-doc-fontsize', _fontIdx);
  }
  _applyDocFont();
...
const _DOC_RECENTS_KEY = 'digital_garage-doc-actions-recent';
...
function _saveSuggestionsToStorage() {
  ...
  if (data.length) {
    localStorage.setItem('digital_garage-suggestions-' + activeDocId, JSON.stringify(data));
  } else {
    localStorage.removeItem('digital_garage-suggestions-' + activeDocId);
  }
}
...
function _restoreSuggestionsFromStorage(docId) {
  try {
    const raw = localStorage.getItem('digital_garage-suggestions-' + docId);
    if (!raw) return;
