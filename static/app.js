// ============================================
// Digital_garage UI — Main Application Orchestrator
...
// IMPORTANT: import dyom.js with NO ?v= query — the same plain specifier
// every other importer (dyom-hwfit.js / dyom-diagnosis.js) uses. A query
// mismatch makes the browser load dyom.js twice as separate modules (two
// _envState objects), which broke server selection. Keep all dyom imports
// unversioned so this can't recur.
import dyomModule from './js/dyom.js';
...
async function _refreshDefaultChat() {
	try {
		...
		if (d && d.endpoint_url && d.model) {
			_defaultChat = d;
			try { window.__digital_garageDefaultChat = d; } catch (_) { }
			return d;
		}
	} catch (_) { }
	return null;
...
function initializeEventListeners() {
	...
	if (exportPdfBtn) {
		exportPdfBtn.addEventListener('click', (e) => {
			...
			const sessionName = meta ? meta.name : 'Garage Chat';
	...
	// ── DYOM modal toggle ──
	const toolDYOMBtn = el('tool-dyom-btn');
	if (toolDYOMBtn) {
		toolDYOMBtn.addEventListener('click', async () => {
			if (!dyomModule) return;
			// Try minimized→restore or open→minimize via the manager first
			const Modals = await import('./js/modalManager.js');
			if (!Modals.toggle('dyom-modal')) {
	...
	// URL-based panel routing — bookmark /calendar, /notes, /dyom etc
	...
	const _routeOpen = {
		...
		'/dyom': () => document.getElementById('tool-dyom-btn')?.click(),
		'/email': () => {
	...
	if (_opener) window._digital_garageRouteOpener = _opener;
	...
	if (modelSortBtn && modelSortDropdown) {
		...
		modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
			opt.addEventListener('click', () => {
				const mode = opt.dataset.sort;
				Storage.set('digital_garage-model-sort', mode);
	...
	const SPLASH_COUNT_KEY = 'digital_garage-tool-splash-counts';
	const SPLASH_MAX = 2;
	const _toolSplashes = {
	...
	(function initModelPickerResponsive() {
		...
		function checkPickerOverflow() {
			...
			if (textarea) {
				textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Garage...');
			}
	...
	const UI_VIS_KEY = 'digital_garage-ui-visibility';
	...
	(function migrateOldToolbarVis() {
		const OLD_KEY = 'digital_garage-toolbar-visibility';
		try {
...
// ============================================
function startDigital_GarageApp() {
	if (window.__digital_garageAppStarted) return;
	window.__digital_garageAppStarted = true;
	// Set CSS variables
	...
	if (documentModule) {
		...
		if (_curSession && localStorage.getItem('digital_garage-doc-open-' + _curSession) === '1') {
	...
	// Sync the contextual rail icons. Tool launchers (calendar/compare/dyom/
	...
	window._digital_garageBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
	...
	if (sessionModule) {
		...
		sessionModule.loadSessions()
			...
			.finally(() => {
				...
				if (window._digital_garageRouteOpener) {
					try { window._digital_garageRouteOpener(); } catch (_) { }
					window._digital_garageRouteOpener = null;
				}
			});
	} else {
...
if (document.readyState === 'loading') {
	document.addEventListener('DOMContentLoaded', startDigital_GarageApp, { once: true });
} else {
	startDigital_GarageApp();
}
