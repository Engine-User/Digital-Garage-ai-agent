  ...
  if (delBtn) {
    delBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (isBuiltin) {
          const hidden = JSON.parse(localStorage.getItem('digital_garage-hidden-presets') || '[]');
          if (!hidden.includes(charName)) hidden.push(charName);
          localStorage.setItem('digital_garage-hidden-presets', JSON.stringify(hidden));
        }
...
function _populateCharSelect() {
  ...
  const hiddenPresets = JSON.parse(localStorage.getItem('digital_garage-hidden-presets') || '[]');
...
function initPersistentChat() {
  ...
  btn.addEventListener('click', async () => {
    ...
    try {
      ...
      const charSessions = JSON.parse(localStorage.getItem('digital_garage-char-sessions') || '{}');
      charSessions[sessionId] = charName;
      localStorage.setItem('digital_garage-char-sessions', JSON.stringify(charSessions));
