  ...
  try {
    ...
    if (entry.model) {
      ...
      if (match) { url = match.url; model = entry.model; epId = match.endpoint_id || ''; }
        ...
        // Endpoint known but not in the live list (e.g. dyom model
...
const _CATEGORY_HUES = [
  ...
  { hue:  35, kw: /\b(dyom|model[-_ ]?(serve|download)|hf|huggingface|vllm|llama|ollama)\b/i }, // amber — dyom
...
const _CATEGORY_LABELS = [
  ...
  { label: 'dyom',  kw: /\b(dyom|model[-_ ]?(serve|download)|hf|huggingface|vllm|llama|ollama)\b/i },
