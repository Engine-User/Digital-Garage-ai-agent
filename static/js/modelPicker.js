...
function _initModelPickerDropdown() {
  ...
  function _populate(filter) {
    ...
    // Load favorites
    const favs = (function () { try { return JSON.parse(localStorage.getItem('digital_garage-model-favorites') || '[]'); } catch { return []; } })();
    ...
    function _addRow(m) {
      ...
      if (m.stale) {
        ...
        row.title = `Local server appears offline: ${m.staleReason}. Click to try anyway, or relaunch in DYOM.`;
      }
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('digital_garage:model-picked', { detail: m })); } catch { }
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `digital_garage-model-favorites[0]`
