...
DIGITAL_GARAGE_MAIL_ORIGIN = "digital_garage-ui"
...
def _apply_digital_garage_headers(msg, kind: str | None = None, ref_id: str | None = None):
    msg["X-digital_garage-Origin"] = DIGITAL_GARAGE_MAIL_ORIGIN
    if kind:
        msg["X-Digital_Garage-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", kind)[:64]
    if ref_id:
        msg["X-Digital_Garage-Ref"] = re.sub(r"[^A-Za-z0-9_.:-]", "-", ref_id)[:128]
...
def setup_email_routes():
    ...
    def _list_emails_sync(folder, limit, offset, filter_, account_id, from_addr=None, has_attachments_only=False, owner=""):
        ...
        try:
            ...
            if filter_ == "unread":
                ...
                # Prefer the DIGITAL_GARAGE marker header, but include the subject
                # fallback too. The fallback uses a distinct DIGITAL_GARAGE prefix
                # so ordinary emails containing "Reminder" don't get mixed in.
                status, data = conn.search(
                    None,
                    f'(OR HEADER X-Digital_Garage-Kind "reminder" SUBJECT "Reminder (Digital_Garage):"{from_clause})',
                )
    ...
    @router.delete("/digital_garage/reminders")
    async def delete_digital_garage_reminder_emails(
        account_id: str | None = Query(None),
        permanent: bool = Query(False),
        owner: str = Depends(require_owner),
    ):
        """Delete email messages stamped as digital_garage reminders."""
        if account_id:
        ...
        try:
            ...
            with _imap(account_id, owner=owner) as conn:
                ...
                for folder_name in candidates:
                    ...
                    try:
                        ...
                        uids.update(_search_uids(conn, f'(HEADER X-Digital_Garage-Kind {_search_quote("reminder")})'))
                        uids.update(_search_uids(conn, f'(SUBJECT {_search_quote("Reminder (Digital_Garage):")})'))
                        for addr in own_addrs:
                            addr_q = _search_quote(addr)
                            uids.update(_search_uids(conn, f'(FROM {addr_q} SUBJECT {_search_quote("Reminder (Digital_Garage):")})'))
                            # Legacy reminders created before the Digital_Garage
            ...
            logger.error(f"delete_digital_garage_reminder_emails failed: {e}")
            return {"success": False, "error": "Mail operation failed"}
    ...
    async def _send_email_sync(
        ...
        account_id=None, owner="", digital_garage_kind=None, digital_garage_ref=None,
    ):
        ...
        _apply_digital_garage_headers(outer, digital_garage_kind or "scheduled", digital_garage_ref)
        if in_reply_to:
    ...
    async def schedule_email(req: dict, owner: str = Depends(require_owner)):
        ...
        try:
            ...
            conn.execute("""
                ...
                (id, to_addr, cc, bcc, subject, body, in_reply_to, references_hdr, attachments, send_at, created_at, status, account_id, digital_garage_kind)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)
            """, (
                sid,
                req.get("to", ""),
                ...
                req.get("digital_garage_kind") or "scheduled",
            ))
            conn.commit()
            conn.close()
    ...
    async def send_email(req: SendEmailRequest, background_tasks: BackgroundTasks, owner: str = Depends(require_owner)):
        ...
        outer["Date"] = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S +0000")
        outer["Message-ID"] = email.utils.make_msgid(domain="digital_garage.local")

        if req.in_reply_to:
            outer["In-Reply-To"] = req.in_reply_to
        if req.references:
            outer["References"] = req.references
        if req.digital_garage_kind:
            _apply_digital_garage_headers(outer, req.digital_garage_kind)
