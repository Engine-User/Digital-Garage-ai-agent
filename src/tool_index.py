...
ALWAYS_AVAILABLE = frozenset({
    ...
    # The two genuinely AMBIENT dyom tools — "what's running" and
    # "kill it" can be asked any time without prior dyom context,
    # and need to survive typos. The other dyom tools (downloads,
    # presets, serve, cached, servers) are CONTEXTUAL — they fire via
    # keyword hints when the user is actually talking about dyom.
...
COLLECTION_NAME = "digital_garage_tool_index"
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "list_served_models": "List currently running model servers in the DYOM — shows status (loading, ready, idle, error), model name, port, throughput, and serve failure diagnosis/retry suggestions. Use when the user asks 'what's running', 'show my dyom', 'which models are up', 'what's serving'.",
    "stop_served_model": "Stop a running model server in the DYOM by session ID or model name. Use when the user says 'kill my dyom', 'stop the model', 'kill the serve', 'shut down vLLM', 'cancel the running model'.",
    "list_downloads": "List in-progress HuggingFace model downloads in the DYOM. Shows model name, phase, percent, session ID. Use for 'what's downloading', 'show my downloads', 'check download progress'.",
    ...
    "list_cached_models": "List models already cached on disk locally or on a remote host. Accepts friendly DYOM server names like ajax. Use for 'what models do I have', 'show cached models', 'is X downloaded', 'list my models'. Avoids re-downloading.",
    "list_serve_presets": "List saved DYOM serve presets (templates with model+host+port+cmd). Always call this BEFORE serve_model when the user asks to launch a known model — they probably have a preset for it from the UI.",
    "serve_preset": "Launch a saved DYOM serve preset by name. Reuses the exact tmux command + host the user already saved. Use for 'run stable diffusion 3.5', 'serve vllm-qwen', 'start the inpaint model' — preset-name matches the user's UI labels.",
    "adopt_served_model": "Register an existing tmux model server (one started manually or outside the dyom flow) into DYOM tracking AND add it as a chat endpoint. Use when the user (or a previous turn) launched something via ssh+tmux and now wants it visible in the UI, stoppable via stop_served_model, and usable in the model picker.",
    "list_dyom_servers": "List the dyom's configured servers (remote GPU boxes + local) and which is the current default. Use this BEFORE download_model/serve_model when the user didn't name a host — to decide where to run, or to ask the user which server when ambiguous. Downloads/serves default to the dyom's selected server, NOT localhost.",
    "app_api": "Generic loopback to ANY digital_garage internal endpoint. Use this when the user wants something the UI can do but there's no named tool for it. Covers calendar, gallery, library/documents, memory, notes, tasks, settings, research, compare, dyom GPUs/state — every UI button hits some /api/* endpoint and you can hit it too. action='endpoints' with filter=<keyword> lists available endpoints. action='call' takes method+path+body. Hits same routes the UI uses — auth flows free. NOTE: themes are NOT an API endpoint — use the ui_control tool (create_theme / set_theme), not app_api. SESSIONS/CHATS: do NOT use app_api for these — GET /api/sessions returns EMPTY for tool calls (it's owner-filtered and tool calls authenticate as a different identity). EMAIL ACCOUNTS: do NOT use /api/email/accounts via app_api; use list_email_accounts, list_emails, and read_email instead. To list/rename/archive/delete/fork chats use the list_sessions and manage_session tools instead.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        frozenset({"theme", "color scheme", "colors of the ui", "make it dark",
            ...
            {"ui_control"},
        # DYOM / model serving intent — user says "kill dyom",
        # "stop the model", "what's running", etc.
        frozenset({"dyom", "kill dyom", "stop dyom",
