...
const _applyImageTool = createApplyImageTool({
	...
	openDYOMForDependency: (pkg) => _openDYOMForDependency(pkg),
	openDYOMForImg2img: () => _openDYOMForImg2img(),
	spinnerModule,
	uiModule,
});
...
function _buildEditor(container) {
	...
	function _wireBrushSlider(el) {
		...
		el.addEventListener("input", (e) => {
			...
			state.brushSize = Math.max(1, Math.round(800 ** (pos / 1000)));
...
// Open the DYOM modal scoped to img2img-capable models so the user
// can serve one in a few clicks. Falls back to plain DYOM if the
// filter hook isn't available.
// Open DYOM on its Dependencies tab and highlight a specific
// package row. Used for "rembg not installed" → install path.
function _openDYOMForDependency(pkgName) {
	// Use dyomModule.open({ tab: 'Dependencies' }) so the intent is
	// honored after DYOM's async render. The old path clicked the
	// sidebar button + polled for the modal, but DYOM's _renderRecipes
	// runs AFTER an awaited _syncFromServer, so depsTab.click() often
	// raced and the user landed on Download.
	const dyom = window.dyomModule;
	if (!dyom || typeof dyom.open !== "function") {
		...
		if (btn) btn.click();
			...
			uiModule.showToast(`Open DYOM to install ${pkgName}`, 6000);
		return;
	}
	dyom.open({ tab: "Dependencies" });
	// Now wait for the Dependencies group to render, switch the server
	// selector to Local, and highlight the package row.
	const cb = document.getElementById("dyom-modal");
	...
	const tryHighlight = (a2 = 0) => {
		...
		if (row) {
			...
			row.classList.add("dyom-pkg-flash");
			setTimeout(() => row.classList.remove("dyom-pkg-flash"), 2000);
		}
	};
	tryHighlight();
}

// Async check whether `rembg` is installed on the digital_garage server.
// Toggles the "install rembg" notice + the Bg Remove run button. The
// `/api/dyom/packages` endpoint is cheap (importlib calls only).
async function _checkRembgInstalled() {
	...
	try {
		const r = await fetch("/api/dyom/packages", {
			credentials: "same-origin",
		});
...
function _openDYOMForImg2img() {
	...
	if (btn && btn.offsetParent !== null) {
		...
		try {
			modalManager.restore("dyom-modal");
			opened = true;
		} catch {}
	}
	if (opened) {
		...
		const tryServe = (attempt = 0) => {
			const cb = document.getElementById("dyom-modal");
			const serveTab = cb
				? cb.querySelector('.dyom-tab[data-backend="Serve"]')
				: null;
			// Retry until BOTH the modal mounts AND its tab bar has rendered.
			// DYOM builds its body html after the modal opens, so we need
	...
	if (uiModule)
		uiModule.showToast(
			"Open DYOM from the sidebar to serve an img2img model",
			6000,
		);
}
...
function _saveProject() {
	...
	const project = {
		v: 1,
		type: "digital_garage-gallery-editor-project",
		imgWidth: state.imgWidth,
		imgHeight: state.imgHeight,
		activeLayerId: state.activeLayerId,
		nextLayerId: state.nextLayerId,
...
function _loadProjectPrompt() {
	...
	inp.addEventListener("change", async () => {
		...
		try {
			...
			if (proj.type !== "digital_garage-gallery-editor-project") {
