...
ATTACHMENTS_DIR = Path(os.environ.get("DIGITAL_GARAGE_MAIL_ATTACHMENTS_DIR", str(DATA_DIR / "mail-attachments")))
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "digital_garage_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN digital_garage_kind TEXT")
    except Exception:
        pass
    ...
    try:
        ...
        try:
            if account_id:
                ...
                # for query-param account_ids, but other callers (dyom
...
class SendEmailRequest(BaseModel):
    ...
    digital_garage_kind: Optional[str] = None
