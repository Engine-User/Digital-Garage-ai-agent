...
import {
  ...
  _serverEntryHtml,
  _copyText,
  // Import dyom.js WITHOUT a ?v= query — the same plain specifier every other
  // importer uses. A query mismatch loads dyom.js twice as two separate modules
...
// fresh scan returns and swaps them in place. Lives here (not dyom.js) because
...
export async function _hwfitFetch(fresh = false) {
  ...
  if (_cached) {
    ...
    // Text label like the other dyom tabs: "Loading…", then if the scan runs
...
export function _expandModelRow(row, modelData) {
  ...
  html += `<button class="dyom-btn hwfit-dl-btn">Download</button>`;
  if (!modelData.is_image_gen) {
    html += `<button class="dyom-btn dyom-run-btn hwfit-quickrun-btn" title="Download + launch with smart defaults">Run</button>`;
    html += `<button class="dyom-btn hwfit-serve-expand-btn" title="Configure & serve">Configure</button>`;
  }
  html += `</div>`;
  if (modelData.is_image_gen) {
    ...
    quickRunBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        if (data.ok) {
          ...
          const runTab = document.querySelector('.dyom-tab[data-backend="Running"]');
...
export function _hwfitInit() {
  ...
  function _syncServers() {
    ...
    entries.forEach(entry => {
      const row = entry.querySelector('.dyom-server-row');
      if (!row) return;
      const nameEl = row.querySelector('.dyom-srv-name');
      const hostEl = row.querySelector('.dyom-srv-host');
      const name = nameEl?.value.trim() || '';
      const host = (hostEl?.disabled || hostEl?.readOnly) ? '' : (hostEl?.value.trim() || '');
      const port = row.querySelector('.dyom-srv-port')?.value.trim() || '';
      const env = row.querySelector('.dyom-srv-env')?.value || 'none';
      const envPath = row.querySelector('.dyom-srv-path')?.value.trim() || '';
      ...
      const dirTags = entry.querySelectorAll('.dyom-modeldir-tag');
      const modelDirs = [];
      dirTags.forEach(tag => {
    ...
    // dropdown still showed digital_garage. The user's selection must only change via
  ...
  async function _testServerConnection(entry) {
    ...
    if (!dot) return;
    if (!host) {
      dot.className = 'dyom-srv-status';
      dot.title = 'Enter user@host to test';
      setMsg('');
      return;
    }
    dot.className = 'dyom-srv-status testing';
  ...
  async function _fetchDYOMSshKey(generate = false) {
    const res = await fetch('/api/dyom/ssh-key', {
  ...
  async function _populateServerKeyPanel(entry, generate = false) {
    const panel = entry.querySelector('.dyom-server-key-panel');
    const cmdBox = entry.querySelector('.dyom-server-key-command');
    const copyBtn = entry.querySelector('.dyom-server-key-copy');
    const genBtn = entry.querySelector('.dyom-server-key-gen');
    ...
    try {
      let publicKey = await _fetchDYOMSshKey(generate);
      if (!publicKey && !generate) publicKey = await _fetchDYOMSshKey(true);
  ...
  function _wireServerEntry(entry) {
    ...
    const row = entry.querySelector('.dyom-server-row');
    const titleEl = entry.querySelector('.dyom-server-title');
    if (!entry.querySelector('.dyom-srv-status')) {
      ...
      // without an SSH test.
      const _hostEl = entry.querySelector('.dyom-srv-host');
      if (_hostEl && (_hostEl.readOnly || _hostEl.disabled)) {
        dot.className = 'dyom-srv-status ok';
        dot.title = 'Local (this machine)';
      }
    }
    const checkBtn = entry.querySelector('.dyom-server-check-btn');
    ...
    // server is what DYOM lands on (all dropdowns) on the next open.
    const _defBtn = entry.querySelector('.dyom-srv-default');
    if (_defBtn && !_defBtn.dataset.bound) {
      ...
      _defBtn.addEventListener('click', (e) => {
        ...
        _persistEnvState();
        document.querySelectorAll('.dyom-srv-default').forEach(b => {
          ...
          b.innerHTML = (on ? _MODELDIR_CHECK_ON : _MODELDIR_CHECK_OFF) + '<span class="dyom-srv-default-label">default</span>';
          b.title = on ? 'Default server — DYOM opens here' : 'Make this the default server';
        });
        // Apply immediately so the dropdowns reflect it without reopening
        // (inline — _applyServerSelection lives in dyom.js and isn't imported here).
    ...
    const keyBtn = entry.querySelector('.dyom-server-key-btn');
    ...
    const keyGenBtn = entry.querySelector('.dyom-server-key-gen');
    ...
    const keyCopyBtn = entry.querySelector('.dyom-server-key-copy');
    if (keyCopyBtn && !keyCopyBtn.dataset.bound) {
      keyCopyBtn.dataset.bound = '1';
      keyCopyBtn.addEventListener('click', async () => {
        const cmd = entry.querySelector('.dyom-server-key-command')?.value?.trim() || '';
    ...
    entry.querySelectorAll('input, select').forEach(el => {
      el.addEventListener('change', () => {
        ...
        const entryHost = entry.querySelector('.dyom-srv-host')?.value?.trim() || '';
        _syncServers();
        ...
        if (!entry.querySelector('.dyom-server-key-panel')?.classList.contains('hidden')) {
          _populateServerKeyPanel(entry, false);
        }
      });
    });
    // Auto-test when host or port blur
    entry.querySelectorAll('.dyom-srv-host, .dyom-srv-port').forEach(el => {
      el.addEventListener('blur', () => _testServerConnection(entry));
    });
    // Initial test for pre-filled rows (existing servers on tab load)
    if (entry.querySelector('.dyom-srv-host')?.value?.trim() && !entry.dataset.tested) {
    ...
    const cancelBtn = entry.querySelector('.dyom-server-cancel-btn');
    ...
    const saveBtn = entry.querySelector('.dyom-server-save-btn');
    ...
    // Setup is owned by dyom.js's delegated handler (Settings behavior:
    ...
    if (setupBtn) {
      setupBtn.addEventListener('click', async () => {
        ...
        try {
          const res = await fetch('/api/dyom/setup', {
            method: 'POST', credentials: 'same-origin',
            headers: { 'Content-Type': 'application/json' },
          ...
          if (data.ok) {
            ...
            if (data.platform) {
              ...
              _syncServers();
              // Show platform badge
              const existingBadge = entry.querySelector('.dyom-platform-badge');
              if (existingBadge) existingBadge.remove();
              const badge = document.createElement('span');
              badge.className = 'dyom-platform-badge';
            ...
            if (data.platform === 'termux') {
              const container = entry.querySelector('.dyom-modeldirs');
              if (container) {
                const existing = [...container.querySelectorAll('.dyom-modeldir-tag')].map(t => t.textContent.replace('\u2716', '').replace('\u2715', '').trim());
                const termuxDir = '/data/data/com.termux/files/home/models';
                if (!existing.includes(termuxDir)) {
                  const tag = document.createElement('span');
                  tag.className = 'dyom-modeldir-tag';
                  tag.dataset.dirIdx = existing.length;
                  tag.innerHTML = `${uiModule.esc(termuxDir)} <span class="dyom-modeldir-rm" title="Remove">\u2715</span>`;
                  tag.querySelector('.dyom-modeldir-rm').addEventListener('click', () => { tag.remove(); _syncServers(); });
                  const addBtn = container.querySelector('.dyom-modeldir-add');
    ...
    const addDirBtn = entry.querySelector('.dyom-modeldir-add');
    if (addDirBtn) addDirBtn.addEventListener('click', () => {
      ...
      const existing = [...entry.querySelectorAll('.dyom-modeldir-tag')].some(t => (t.dataset.dir || t.textContent.trim()) === dir);
      if (existing) return;
      const container = entry.querySelector('.dyom-modeldirs');
      const tag = document.createElement('span');
      tag.className = 'dyom-modeldir-tag';
      tag.dataset.dirIdx = container.querySelectorAll('.dyom-modeldir-tag').length;
      tag.dataset.dir = dir;
      tag.innerHTML = `<span class="dyom-modeldir-dl" title="Send downloads here" data-dl-dir="${uiModule.esc(dir)}">${_MODELDIR_CHECK_OFF}</span> ${uiModule.esc(dir)} <span class="dyom-modeldir-rm" title="Remove">\u2716</span>`;
      tag.querySelector('.dyom-modeldir-rm').addEventListener('click', () => { tag.remove(); _syncServers(); });
      _wireModelDirTarget(entry, tag.querySelector('.dyom-modeldir-dl'));
      container.insertBefore(tag, addDirBtn);
      _syncServers();
    });
    entry.querySelectorAll('.dyom-modeldir-rm').forEach(rm => {
      rm.addEventListener('click', () => { rm.closest('.dyom-modeldir-tag').remove(); _syncServers(); });
    });
    ...
    entry.querySelectorAll('.dyom-modeldir-dl').forEach(dl => _wireModelDirTarget(entry, dl));
  }
  ...
  function _wireModelDirTarget(entry, dlEl) {
    if (!dlEl) return;
    const tag = dlEl.closest('.dyom-modeldir-tag');
    if (!tag || tag.dataset.dlBound) return;
    tag.dataset.dlBound = '1';
    tag.style.cursor = 'pointer';
    tag.addEventListener('click', (e) => {
      if (e.target.closest('.dyom-modeldir-rm')) return;   // remove handled elsewhere
      e.stopPropagation();
      entry.querySelectorAll('.dyom-modeldir-dl').forEach(d => {
        d.classList.remove('active');
        d.innerHTML = _MODELDIR_CHECK_OFF;          // uncheck the others
        d.closest('.dyom-modeldir-tag')?.classList.remove('dyom-modeldir-target');
        d.title = 'Send downloads here';
      });
      dlEl.classList.add('active');
      dlEl.innerHTML = _MODELDIR_CHECK_ON;           // check the chosen one
      tag.classList.add('dyom-modeldir-target');
  ...
  document.querySelectorAll('.dyom-server-entry').forEach(_wireServerEntry);

  const addBtn = document.getElementById('dyom-server-add');
  if (addBtn && !addBtn.dataset.bound) {
    addBtn.dataset.bound = '1';
    addBtn.addEventListener('click', () => {
      const list = document.getElementById('dyom-servers-list');
      if (!list) return;
      ...
      _rebuildServerSelect();
      entry.querySelector('.dyom-srv-host')?.focus();
    });
  }
