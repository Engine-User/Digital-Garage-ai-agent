...
const _rememberedDockKey = (id) => `digital_garage-modal-remembered-dock-${id}`;
...
// = 250, dyom/theme = 260, …), so restoring one from the dock could leave
...
function _emitModalOpened(id, modal) {
  try {
    window.dispatchEvent(new CustomEvent('digital_garage:modal-opened', {
      detail: { id, modal },
    }));
  } catch (_) { }
}
...
const _LABELS = {
  'dyom-modal': { label: 'DYOM', icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 7v14"/><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"/></svg>' },
...
const _DOCK_STORAGE_KEY = 'digital_garage.mobileDockState.v1';
...
export function close(id) {
  ...
  // Some tools (dyom) animate their close over ~250ms before adding
...
export function injectMinimizeButton(modal, modalId) {
  ...
  // justify-content. Some headers (dyom) use `space-between`, which
...
const _AUTO_WIRE = {
  'dyom-modal': { rail: 'rail-dyom', sidebar: 'tool-dyom-btn' },
...
// etc.) when the dyom modal is swiped away. Otherwise the dropdowns
// stay floating in the middle of the page with no anchor.
window.addEventListener('modal-dismissed', (e) => {
  const id = e.detail?.id;
  if (id === 'dyom-modal') {
    document.querySelectorAll(
      '.dyom-task-dropdown, .dyom-gpu-split-menu, .hwfit-cached-dropdown, .dyom-saved-menu, .dyom-dep-menu'
    ).forEach(d => d.remove());
  }
});
