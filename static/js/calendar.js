...
const BADGE_SEEN_KEY = 'digital_garage-calendar-badge-seen';
...
function _wireAll(body) {
  ...
  try {
    ...
    if (calBody && splitter) {
      ...
      if (!alreadySet) {
        const saved = parseInt(localStorage.getItem('digital_garage.cal.detailH') || '0', 10);
      ...
      const onUp = () => {
        ...
        if (px) { try { localStorage.setItem('digital_garage.cal.detailH', String(px)); } catch { } }
      };
      const onDown = (ev) => {
        ev.preventDefault();
        dragging = true;
      ...
      const resetSplit = () => {
        ...
        try { localStorage.removeItem('digital_garage.cal.detailH'); } catch { }
      };
...
const LS_KEY = 'digital_garage-calendar-cache';
