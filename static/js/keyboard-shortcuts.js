...
const _defaultKeybinds = {
  ...
  open_calendar: 'ctrl+alt+c', open_compare: '', open_dyom: '',
...
export function initKeyboardShortcuts(modules) {
  ...
  window._digital_garageKeybinds = { ..._defaultKeybinds };

  // Load saved keybinds
  fetch('/api/auth/settings', { credentials: 'same-origin' })
    .then(r => r.json())
    .then(s => { if (s.keybinds) window._digital_garageKeybinds = { ..._defaultKeybinds, ...s.keybinds }; })
    .catch(() => { });
  ...
  // email, dyom serve — without each module wiring its own listener.
  ...
  document.addEventListener('keydown', (e) => {
    const kb = window._digital_garageKeybinds;
      ...
      uiModule.styledConfirm('Delete this session?', { confirmText: 'Delete', danger: true }).then(ok => {
        ...
        fetch(`${API_BASE}/api/session/${sid}`, { method: 'DELETE' }).then(async () => {
          ...
          if (nextSession) {
            ...
            el('current-meta').textContent = 'Digital Garage Chat';
