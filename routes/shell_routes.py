...
TMUX_LOG_DIR = "/tmp/tmux_logs"   # keep this at module level, no pty needed
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f"DIGITAL_GARAGE_USER_SHELL=\"${{SHELL:-}}\"\n"
        f"if [ -n \"$DIGITAL_GARAGE_USER_SHELL\" ] && [ -x \"$DIGITAL_GARAGE_USER_SHELL\" ]; then\n"
        f"  DIGITAL_GARAGE_USER_PATH=\"$(\"$DIGITAL_GARAGE_USER_SHELL\" -ic 'printf \"__DIGITAL_GARAGE_PATH__%s\\n\" \"$PATH\"' 2>/dev/null | sed -n 's/^__DIGITAL_GARAGE_PATH__//p' | tail -n 1 || true)\"\n"
        f"  if [ -n \"$DIGITAL_GARAGE_USER_PATH\" ]; then export PATH=\"$DIGITAL_GARAGE_USER_PATH:$PATH\"; fi\n"
        f"fi\n"
...
def setup_shell_routes() -> APIRouter:
    ...
    async def list_packages(host: str | None = None, ssh_port: str | None = None, venv: str | None = None):
        ...
        packages = [
            ...
            {"name": "tmux", "pip": "", "desc": "Required for Linux/Termux DYOM background downloads and serves", "category": "System", "target": "remote", "kind": "system", "install_hint": "Run DYOM server setup, or install tmux with apt/pacman/dnf/apk/zypper."},
    ...
    @router.post("/api/dyom/packages/install")
