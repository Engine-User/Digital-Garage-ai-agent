...
# DYOM routes loopback. The agent's tool calls run in-process but
# need to reach admin-gated dyom routes; we ride the per-process
# internal token so require_admin lets us through. See core/middleware.py.
_DYOM_BASE = "http://localhost:7000"
...
def _internal_headers(owner: Optional[str] = None) -> Dict[str, str]:
    ...
    if owner:
        headers["X-Digital_Garage-Owner"] = owner
    return headers


async def _dyom_servers() -> Dict[str, Any]:
    """Return the dyom's configured servers + the currently-selected
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(f"{_DYOM_BASE}/api/dyom/state", headers=_internal_headers())
...
async def _resolve_dyom_host(name_or_host: str) -> str:
...
async def _dyom_env_for_host(host: str) -> Dict[str, Any]:
    """Resolve env_prefix / gpus / platform / hf_token / ssh_port for a
    given host by looking it up in dyom_state.env. The user
    configures these per-host in the DYOM UI; without them, raw
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.get(f"{_DYOM_BASE}/api/dyom/state", headers=headers)
            state = r.json() if r.headers.get("content-type", "").startswith("application/json") else {}
    except Exception as e:
        logger.debug(f"dyom env lookup failed for host={host!r}: {e}")
        return {}
...
async def _dyom_register_task(session_id: str, model: str, host: str,
                                  cmd: str, task_type: str = "serve") -> bool:
    """Append a task entry to dyom_state.json after the agent
    ...
    do that here so the task shows up in the DYOM tab.
    ...
    try:
        ...
        logger.debug(f"dyom state read failed: {e}")
        return False
    if not isinstance(state, dict):
        state = {}
    ...
    # Placeholder output — the dyom UI's CSS hides empty <pre>
    # via `.dyom-output-pre:empty { display: none }`, so an
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(f"{_DYOM_BASE}/api/dyom/state",
                                  json=state, headers=headers)
        return r.status_code < 400
    except Exception as e:
        logger.debug(f"dyom state write failed: {e}")
        return False
...
# dyom_state.json (presets + tasks) by POSTing {"tasks": []} to
# /api/dyom/state, which overwrote the whole file. Use the
...
_APP_API_BLOCKLIST_METHOD_PATH = (
    ...
    ("POST",   "/api/dyom/state"),   # whole-file overwrite — agent must use serve_preset/serve_model instead
    ("DELETE", "/api/dyom/state"),
    # Use the named tools (download_model / serve_model) — they handle
    # host-name resolution, per-host env_prefix, AND register the task
    # in dyom state so it shows in the UI + list_downloads. Hitting
...
async def do_app_api(content: str, owner: Optional[str] = None) -> Dict:
    """Generic loopback to any internal Digital_Garage API endpoint. Lets the
    agent reach the full UI-button surface (dyom, email, notes,
    ...
    Args (JSON):
      ...
      path:   "/api/dyom/gpus"     # required for call
    ...
    if not path:
        return {"error": "path is required (e.g. '/api/dyom/gpus')", "exit_code": 1}
    ...
    if any(method == m and path.startswith(p) for m, p in _APP_API_BLOCKLIST_METHOD_PATH):
        ...
        if "/api/model/serve" in path:
            return {"error": "Don't POST /api/model/serve directly — use the `serve_model` or `serve_preset` tool (handles host resolution, env_prefix, and dyom tracking).", "exit_code": 1}
        ...
        return {"error": f"{method} {path} is blocked — it overwrites the whole dyom state file. Use list_serve_presets / serve_preset / serve_model instead.", "exit_code": 1}
...
# the dyom's task tracker. Each entry: (label, substring-list).
...
def _dyom_apply_retry_suggestion(cmd: str, suggestion: Dict[str, Any]) -> str:
    """Apply a structured DYOM diagnosis suggestion to a serve command."""
...
def _scan_running_model_processes() -> List[Dict[str, Any]]:
    ...
    a dict shaped like a dyom task so the caller can merge cleanly.
    """
    import os
...
async def do_download_model(content: str, owner: Optional[str] = None) -> Dict:
    """Download a HuggingFace model via the dyom API."""
    import httpx
    try:
        args = _parse_tool_args(content)
    except ValueError:
    ...
    if host:
        host = await _resolve_dyom_host(host)
    # No host specified → default to the dyom's currently-selected
    ...
    # Per-host env_prefix + hf_token from dyom_state (same as serve).
    env_cfg = await _dyom_env_for_host(host)
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"{_DYOM_BASE}/api/model/download",
                                     json=payload, headers=_internal_headers())
            data = resp.json()
        if data.get("ok"):
            sid = data.get("session_id", "?")
            registered = await _dyom_register_task(
            ...
            where = host or "local"
            default_note = " (defaulted to the dyom's selected server — pass host= or local=true to override)" if _host_defaulted else ""
...
async def do_serve_model(content: str, owner: Optional[str] = None) -> Dict:
    """Start serving a model via the dyom API."""
    import httpx
    try:
        args = _parse_tool_args(content)
    except ValueError:
    ...
    # hf_token, platform, ssh_port) from dyom_state — same path
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"{_DYOM_BASE}/api/model/serve",
                                     json=payload, headers=_internal_headers())
            data = resp.json()
        if data.get("ok"):
            sid = data.get("session_id", "?")
...
async def do_list_served_models(content: str, owner: Optional[str] = None) -> Dict:
    """List running model servers — merges dyom-tracked tasks with
    a /proc scan for externally-launched LLM/diffusion processes
    (vLLM, sglang, llama.cpp, Ollama, ComfyUI, A1111, Fooocus, etc.)."""
    import asyncio
    import httpx

    # DYOM-tracked tasks (best-effort; don't fail the whole call if
    # this is unreachable).
    dyom_tasks: List[Dict[str, Any]] = []
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.get(f"{_DYOM_BASE}/api/dyom/tasks/status",
                                    headers=_internal_headers())
            dyom_tasks = (resp.json() or {}).get("tasks") or []
    except Exception as e:
        logger.debug(f"dyom tasks/status fetch failed: {e}")
    ...
    merged.extend(dyom_tasks)
    # Dedupe: if a process's PID is already mentioned by a dyom task
    # (dyom may track the PID via session_id), skip it.
    dyom_pids = set()
    for t in dyom_tasks:
        if isinstance(t, dict) and t.get("pid"):
            dyom_pids.add(t["pid"])
    for p in external:
        if p.get("pid") not in dyom_pids:
            merged.append(p)

    if not merged:
        return {
            "output": "No model servers currently running (dyom task tracker empty; /proc scan found no vLLM / sglang / llama.cpp / Ollama / ComfyUI / A1111 / Fooocus / InvokeAI / TGI / Aphrodite / Triton / Diffusers processes).",
            "exit_code": 0,
        }
    ...
    header = []
    if cb_n:
        header.append(f"{cb_n} dyom-tracked")
    if ext_n:
    ...
    for t in merged:
        ...
        if diag:
            ...
            for s in suggestions[:3]:
                ...
                retry_cmd = _dyom_apply_retry_suggestion(cmd, s)
...
async def _dyom_kill_session(session_id: str, *, remote_host: str = "",
                                 ssh_port: str = "", verb: str = "Stopped") -> Dict:
    """Kill a dyom tmux session — remote-aware — AND mark the task
    stopped in dyom_state.json. Shared by stop_served_model and
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(f"{_DYOM_BASE}/api/dyom/state", headers=headers)
            state = resp.json() or {}
    except Exception as e:
        logger.debug(f"dyom state lookup failed for {session_id}: {e}")
    if not isinstance(state, dict):
        state = {}
    matched = None
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.post(f"{_DYOM_BASE}/api/shell/exec",
        ...
        if matched is not None:
            try:
                ...
                async with httpx.AsyncClient(timeout=10) as client:
                    await client.post(f"{_DYOM_BASE}/api/dyom/state",
                                      json=state, headers=headers)
            except Exception as e:
...
async def do_stop_served_model(content: str, owner: Optional[str] = None) -> Dict:
    ...
    return await _dyom_kill_session(
        session_id,
...
async def do_list_downloads(content: str, owner: Optional[str] = None) -> Dict:
    """List in-flight model downloads (filters /api/dyom/tasks/status to type=download)."""
    import httpx
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.get(f"{_DYOM_BASE}/api/dyom/tasks/status",
...
async def do_search_hf_models(content: str, owner: Optional[str] = None) -> Dict:
    """Search HuggingFace via the dyom /api/dyom/hf-latest endpoint."""
    import httpx
    try:
        args = _parse_tool_args(content)
    except ValueError:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.get(f"{_DYOM_BASE}/api/dyom/hf-latest",
...
async def do_adopt_served_model(content: str, owner: Optional[str] = None) -> Dict:
    ...
    anything else) into the DYOM so it appears in list_served_models,
    ...
    the dyom's serve flow but you want first-class tracking.
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            r = await client.post(f"{_DYOM_BASE}/api/shell/exec",
    ...
    # Read+modify+write dyom state. APPEND a task entry; do NOT
    ...
    try:
        ...
        return {"error": f"could not read dyom state: {e}", "exit_code": 1}
    if not isinstance(state, dict):
        state = {}
    tasks = state.get("tasks") if isinstance(state.get("tasks"), list) else []
    ...
    if any(isinstance(t, dict) and t.get("sessionId") == sess for t in tasks):
        ...
        new_task = {
            ...
            "ts": int(_time.time() * 1000),
            "payload": {"repo_id": model, "remote_host": host or "", "_cmd": "(adopted — launched outside dyom)"},
            "remoteHost": host or "",
            "sshPort": "",
            "platform": "linux",
        ...
        try:
            ...
            return {"error": f"could not save dyom state: {e}", "exit_code": 1}
    ...
    return {
        "output": (
            ...
            + ("Already tracked — skipped state write. " if adopted_already else "Added to dyom state. ")
...
async def do_list_dyom_servers(content: str, owner: Optional[str] = None) -> Dict:
    """List the dyom's configured servers and which one is the
    ...
    if not hosts:
        return {"output": "No dyom servers configured. Downloads/serves default to localhost.", "servers": [], "default_host": "", "exit_code": 0}
...
async def do_list_serve_presets(content: str, owner: Optional[str] = None) -> Dict:
    """List saved serve presets from dyom_state.json. Each preset
    ...
    try:
        async with httpx.AsyncClient(timeout=10) as client:
            resp = await client.get(f"{_DYOM_BASE}/api/dyom/state",
                                    headers=_internal_headers())
            state = resp.json() or {}
    except Exception as e:
        return {"error": f"Failed to fetch dyom state: {e}", "exit_code": 1}

    presets = state.get("presets") or []
    if not presets:
        return {
            "output": "No serve presets saved. Tell the user to save one from the DYOM UI first, or use serve_model with explicit repo_id + cmd + host.",
            "presets": [],
            "exit_code": 0,
        }
...
async def do_serve_preset(content: str, owner: Optional[str] = None) -> Dict:
    ...
    cmd + host + model from dyom_state.json, then calls the
    ...
    # etc. from dyom_state.env so launches actually find vllm.
    env_cfg = await _dyom_env_for_host(host)
    if env_cfg.get("env_prefix"): payload["env_prefix"] = env_cfg["env_prefix"]
    if env_cfg.get("gpus"):       payload["gpus"]       = env_cfg["gpus"]
    if env_cfg.get("hf_token"):   payload["hf_token"]   = env_cfg["hf_token"]
...
async def do_list_cached_models(content: str, owner: Optional[str] = None) -> Dict:
    ...
    host = await _resolve_dyom_host(raw_host) if raw_host else ""
    if host:
        params["host"] = host
    ...
    try:
        async with httpx.AsyncClient(timeout=60) as client:
            resp = await client.get(f"{_DYOM_BASE}/api/model/cached",
                                    params=params, headers=_internal_headers())
            data = resp.json()
        models = data.get("models", []) if isinstance(data, dict) else data
        if not models:
            ...
            # Still surface completed DYOM downloads so the agent doesn't
            # incorrectly assume a model is absent and re-download it.
            downloaded = []
            try:
                async with httpx.AsyncClient(timeout=10) as client:
                    st = await client.get(f"{_DYOM_BASE}/api/dyom/state", headers=_internal_headers())
            ...
            if downloaded:
                ...
                lines = [f"No cache paths were detected{host_str}, but DYOM has completed download task(s):"]
                lines.extend(f"- {repo} — downloaded via DYOM task" for repo in downloaded)
                return {"output": "\n".join(lines), "models": [{"repo_id": repo, "source": "dyom_task"} for repo in downloaded], "exit_code": 0}
            host_str = f" on {raw_host or host}" if (raw_host or host) else ""
...
async def do_trigger_research(content: str, owner: Optional[str] = None) -> Dict:
    ...
    try:
        async with httpx.AsyncClient(timeout=30) as client:
            resp = await client.post(f"{_DYOM_BASE}/api/research/start",
