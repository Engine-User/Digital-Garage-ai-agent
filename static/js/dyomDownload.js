...
export function _wirePanelEvents(panel, model, backend) {
  ...
  if (stopBtn) {
    stopBtn.addEventListener('click', () => {
      if (panel._dyomAbort) panel._dyomAbort.abort();
    });
  }

  // Kill & close output button
  const killBtn = panel.querySelector('.dyom-output-kill');
  if (killBtn) {
    killBtn.addEventListener('click', () => {
      if (panel._dyomAbort) panel._dyomAbort.abort();
      const outputText = panel.querySelector('.dyom-output-pre')?.textContent || '';
      const tmuxMatch = outputText.match(/Started tmux session: (dyom-[a-f0-9]+)/);
      if (tmuxMatch) {
      ...
      const wrap = panel.querySelector('.dyom-output-wrap');
      if (wrap) wrap.classList.add('hidden');
      const output = panel.querySelector('.dyom-output-pre');
  ...
  // Output copy button
  const outputCopyBtn = panel.querySelector('.dyom-output-copy');
  if (outputCopyBtn) {
    outputCopyBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      const text = panel.querySelector('.dyom-output-pre')?.textContent || '';
      _copyText(text).then(() => {
...
export async function _runPanelCmd(panel, cmd, opts = {}) {
  const outputWrap = panel.querySelector('.dyom-output-wrap');
  const output = panel.querySelector('.dyom-output-pre');
  if (outputWrap) outputWrap.classList.remove('hidden');
  output.classList.remove('dyom-output-error');
  output.textContent = '';
  _clearDiagnosis(panel);

  const controller = new AbortController();
  panel._dyomAbort = controller;
  ...
  try {
    ...
    if (!res.ok) {
      output.classList.add('dyom-output-error');
...
export async function _runModelDownload(panel, model, backend, hostOverride) {
  ...
  // here: there can be more than one copy of the dyom state in memory and
