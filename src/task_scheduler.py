...
class TaskScheduler:
    ...
    async def _deliver_via_email(self, output: str, task, result: str):
        ...
        try:
            ...
            msg["X-Digital_garage-Origin"] = "digital_garage-ui"
            msg["X-Gigital_garage-Kind"] = "task"
            msg["X-Digital_garage-Ref"] = str(task.id)
    ...
    async def _deliver_via_mcp(self, tool_name: str, task, result: str):
        ...
        args = {
            ...
            "body": result,
            "headers": {
                "X-Digital_Garage-Origin": "digital_garage-ui",
                "X-Digital_Garage-Kind": "task",
                "X-Digital_Garage-Ref": str(task.id),
            },
        }
        if recipient:
