                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Digital_Garage (if public URL is configured).
                                    ...
                                    alert_body = (
                                        ...
                                        f"Reason: {reason}\n\n"
                                        + (f"Open in Digital_Garage: {open_url}\n\n" if open_url else "")
                                        + f"---\n"
                                        f"From: {sender}\n"
                                        f"Subject: {subject}\n"
                                        f"Date: {msg.get('Date','')}\n\n"
                                    ...
                                    # HTML alternative with a clickable "Open in Digital_Garage" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "digital_garage_kind" if "digital_garage_kind" in cols else "'scheduled' AS digital_garage_kind"
        rows = conn.execute(f"""
        ...
        for r in rows:
            ...
            try:
                ...
                digital_garage_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Digital_Garage-Origin"] = "digital_garage-ui"
                outer["X-Digital_Garage-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", digital_garage_kind or "scheduled")[:64]
                outer["X-Digital_Garage-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `Digital_Garage-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `Digital_Garage_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `digital_garage-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("DIGITAL_GARAGE_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `DIGITAL_GARAGE_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (DIGITAL_GARAGE_INPROCESS_POLLERS=0); "
            "drive `digital_garage-mail poll-scheduled` externally."
        )
        return
    import asyncio
