...
const _SETTINGS_KEY = 'digital_garage-research-settings';
const _COLLAPSE_KEY = 'digital_garage-research-settings-collapsed';
...
// Live sidebar/rail feedback — mirrors the dyom pattern. While
...
function _syncResearchRail() {
  ...
  if (toolBtn) {
    ...
    // same style as DYOM's running indicator (no glow).
  ...
  return `
    ...
    <div class="modal-body research-pane-body" data-no-swipe-dismiss>
      <div class="research-new-job">
        ...
        <textarea id="research-query" class="research-query" placeholder="e.g. Trace Digital_Garage's ten-year journey home from Troy — every island, monster, and detour, and why each one cost him" rows="4"></textarea>
...
/** Fade/slide a card out, then run the removal — matches dyom's smooth exit. */
...
function _wireEvents(pane) {
  ...
  pane.querySelector('#research-settings-toggle').addEventListener('click', () => {
    ...
    try { localStorage.setItem('digital_garage-research-settings-collapsed', _settingsCollapsed ? '1' : '0'); } catch { }
  });
...
function _renderJobs() {
  ...
  const _addSection = (key, title, arr) => {
    ...
    // Both sections carry a "Clear all" button in the header (dyom-running
