...
import {
  ...
  _serveAutoFix,
  // Plain specifier (no ?v=) — must match every other dyom.js importer so the
  // browser loads it once. See dyom-hwfit.js.
} from './dyom.js';
...
export const ERROR_PATTERNS = [
  ...
  },
  {
    ...
    message: 'SGLang is not installed or not in PATH. Open DYOM → Dependencies and install sglang on this server.',
    fixes: [
...
export function _showDiagnosis(panel, diagnosis, sourceText) {
  ...
  let diag = panel.querySelector('.dyom-diagnosis');
  if (!diag) {
    diag = document.createElement('div');
    diag.className = 'dyom-diagnosis';
  ...
  msg.className = 'dyom-diag-message';
  ...
  if (diagnosis.fixes && diagnosis.fixes.length) {
    ...
    row.className = 'dyom-diag-fixes';
    for (const fix of diagnosis.fixes) {
      const btn = document.createElement('button');
      btn.className = 'dyom-btn dyom-diag-btn';
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          ...
          console.error('[dyom] diagnosis fix failed', e);
        } finally {
...
export function _clearDiagnosis(panel) {
  ...
  const diag = panel.querySelector('.dyom-diagnosis');
