...
const _ACTION_RECENTS_KEY = 'digital_garage-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'digital_garage-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Digital_garage';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Digital_garage' : shortModel(resolvedModel);
