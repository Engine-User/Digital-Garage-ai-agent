...
def setup_gallery_routes() -> APIRouter:
    ...
    async def inpaint_proxy(request: Request):
        ...
        if not base:
            ...
            try:
                ...
                if not eps:
                    raise HTTPException(400, "No image generation endpoint configured. Serve a diffusion model via DYOM first.")
    ...
    async def harmonize_image(request: Request):
        ...
        if "api.openai.com" in base:
            raise HTTPException(400,
                ...
                "one. DYOM → Models can serve an SD-compatible model "
    ...
    # error so the client can prompt the user to install via DYOM.
    ...
    async def denoise_image(request: Request):
        ...
        try:
            ...
            return {"error": "realesrgan not installed. Install it from DYOM → Dependencies (search 'realesrgan')."}
        try:
