...
if ('ontouchstart' in window) {
  ...
  function _closeFloatingDropdownsForSwipe() {
    document.querySelectorAll(
      '.email-card-dropdown, .hwfit-cached-dropdown, .dyom-saved-menu, .dyom-dep-menu'
    ).forEach(d => {
      if (d._anchor) d._anchor.classList.remove('dyom-menu-active', 'reader-more-active');
      d.remove();
    });
  }
  ...
  document.addEventListener('touchmove', (e) => {
    ...
    if (!_dragging) {
      ...
      if (Math.abs(dy) > 8) {
        ...
        // email reader More menu, dyom serve kebab + saved-configs, and
...
if (!window._odyEscExpandGuard) {
  ...
  // stacking falls back to DOM order — which is unpredictable (dyom is
  ...
  // dyom can render compare UNDER it. Bumping the z-index on every
