...
// "dyom-task-status" ('' = the neutral loading style).
function _taskBadge(task) {
	if (task._unreachable && task.status === 'running') return { text: 'unreachable', cls: 'dyom-task-error' };
	if (task.type === 'serve' && task.status === 'running' && task.progress) {
		...
		return { text: task.progress, cls: 'dyom-task-running' };
	}
	return { text: _statusLabel(task.status, task.type), cls: 'dyom-task-' + task.status };
}
...
// ── Phase detection (mirrors Python _parse_serve_phase in dyom_routes.py) ──
...
const _REMOVED_KEY = 'dyom-removed-tasks';
...
export function _addTask(sessionId, name, type, payload) {
	...
	if (body) {
		const tab = body.querySelector('.dyom-tab[data-backend="Running"]');
		if (tab) tab.click();
	}
	return task;
}
...
function _updateTask(sessionId, updates) {
	...
	if (updates.status && updates.status !== 'running') {
		const el = document.querySelector(`.dyom-task[data-task-id="${sessionId}"]`);
		if (el) {
			if (el._uptimeInterval) { clearInterval(el._uptimeInterval); el._uptimeInterval = null; }
			const wave = el.querySelector('.dyom-task-wave');
			if (wave) wave.style.display = 'none';
			const uptime = el.querySelector('.dyom-task-uptime');
...
function _winSessionCmd(task, tmuxArgs) {
	const sd = '$env:TEMP\\digital_garage-sessions';
...
function _showDYOMNotif(isError = false) {
	const dot = document.getElementById('dyom-notif-dot');
	if (dot) {
		dot.style.display = '';
		dot.classList.toggle('dyom-notif-error', isError);
	}
	const btn = document.getElementById('tool-dyom-btn');
	if (btn) { btn.style.opacity = '1'; btn.classList.add('dyom-notif-active'); }
	const railBtn = document.getElementById('rail-dyom');
	if (railBtn) {
		railBtn.classList.remove('rail-notify-success', 'rail-notify-error');
		railBtn.classList.add('rail-notify', isError ? 'rail-notify-error' : 'rail-notify-success', 'dyom-notif-active');
	}
	if (window._syncRailDynamic) window._syncRailDynamic();
}

export function _clearDYOMNotif() {
	const dot = document.getElementById('dyom-notif-dot');
	if (dot) dot.style.display = 'none';
	const btn = document.getElementById('tool-dyom-btn');
	if (btn) { btn.style.opacity = ''; btn.classList.remove('dyom-notif-active'); }
	const railBtn = document.getElementById('rail-dyom');
	if (railBtn) {
		railBtn.classList.remove('rail-notify', 'rail-notify-success', 'dyom-notif-active');
	}
	if (window._syncRailDynamic) window._syncRailDynamic();
}
...
// Same model-matching as dyomServe's _presetsForModel, so the auto-save cap
...
export async function _syncFromServer() {
	try {
		const res = await fetch('/api/dyom/state', { credentials: 'same-origin' });
		...
		if (state.env) {
			...
			localStorage.setItem('dyom-last-state', JSON.stringify(safeState));
		}
		if (state.presets) {
...
async function _retryTask(el, task) {
	...
	const badge = el?.querySelector('.dyom-task-status');
	if (badge) { badge.textContent = 'restarting...'; badge.className = 'dyom-task-status'; }
	try {
...
export async function _serveAutoFix(panel, envVar) {
	const taskEl = panel.closest('.dyom-task');
	if (!taskEl) return;
...
async function _openServeEditForTask(task, cmdOverride) {
	...
	try {
		const { openServePanelForRepo } = await import('./dyomServe.js');
		await openServePanelForRepo(repo, fields);
	} catch (err) {
		console.error('[dyom] open serve panel failed', err);
...
function _promptEditServeCmd(currentCmd) {
	return new Promise((resolve) => {
		...
		overlay.className = 'dyom-edit-overlay';
		overlay.innerHTML = `
      <div class="dyom-edit-modal">
        <div class="dyom-edit-title">Edit serve command</div>
        <textarea class="dyom-edit-textarea" spellcheck="false"></textarea>
        <div class="dyom-edit-actions">
          <button class="dyom-edit-cancel memory-toolbar-btn">Cancel</button>
          <button class="dyom-edit-save memory-toolbar-btn">Save &amp; relaunch</button>
        </div>
      </div>`;
		const ta = overlay.querySelector('.dyom-edit-textarea');
		ta.value = currentCmd || '';
		...
		overlay.querySelector('.dyom-edit-cancel').addEventListener('click', () => close(null));
		overlay.querySelector('.dyom-edit-save').addEventListener('click', () => close(ta.value.trim() || null));
...
// Mirrors the regex parser in dyomServe.js's _loadSlotIntoPanel.
...
export async function _launchServeTask(shortName, repo, cmd, fields, hostOverride) {
	...
	try {
		...
		if (!data.ok) {
			...
			console.error('[dyom] /api/model/serve failed', { status: res.status, body: data });
...
export function _renderRunningTab() {
	...
	// are actively running or errored. _showDYOMNotif fires on each task
	...
	try {
		...
		if (!_activeTasks.length) _clearDYOMNotif();
	} catch { }
	...
	// had open. Task output: presence of .dyom-task-collapsed means collapsed.
	...
	body.querySelectorAll('.dyom-task').forEach(tEl => {
		const id = tEl.dataset.taskId;
		if (!id) return;
		const wrap = tEl.querySelector('.dyom-output-wrap');
		if (!wrap) return;
		if (wrap.classList.contains('dyom-task-collapsed')) _collapsedTaskIds.add(id);
	...
	body.querySelectorAll('.dyom-section-body').forEach(sb => {
	...
	let tabBar = body.querySelector('.dyom-tabs');
	if (!tabBar) return;
	let runTab = tabBar.querySelector('.dyom-tab[data-backend="Running"]');
	if (hasContent && !runTab) {
		runTab = document.createElement('button');
		runTab.className = 'dyom-tab';
		runTab.dataset.backend = 'Running';
		const _errCount = tasks.filter(t => t.status === 'error' || t.status === 'crashed').length;
		runTab.innerHTML = `Running <span class="dyom-tab-count">${tasks.length}</span>${_errCount ? `<span class="dyom-tab-error-dot"></span>` : ''}`;
		tabBar.insertBefore(runTab, tabBar.firstChild);
		runTab.addEventListener('click', () => {
			tabBar.querySelectorAll('.dyom-tab').forEach(t => t.classList.remove('active'));
		...
		runTab.innerHTML = tasks.length ? `Running <span class="dyom-tab-count">${tasks.length}</span>${_errCount2 ? '<span class="dyom-tab-error-dot"></span>' : ''}` : 'Running';
		if (!hasContent) {
			if (runTab.classList.contains('active')) {
				const wfTab = tabBar.querySelector('.dyom-tab[data-backend="Search"]');
	...
	let group = body.querySelector('.dyom-group[data-backend-group="Running"]');
	if (hasContent && !group) {
		group = document.createElement('div');
		group.className = 'dyom-group hidden';
		...
		group.innerHTML = '<div class="admin-card" style="flex:1;display:flex;flex-direction:column;overflow:hidden;">' +
			...
			'</div>';
		const firstGroup = body.querySelector('.dyom-group');
	...
	group.querySelectorAll('.dyom-serve-section, .dyom-dl-section').forEach(el => el.remove());
	...
	(_adminCard || group).querySelectorAll('[class*="dyom-server-section-"]').forEach(el => {
		const cls = [...el.classList].find(c => c.startsWith('dyom-server-section-'));
		if (cls && !_liveSafeKeys.has(cls.replace('dyom-server-section-', ''))) el.remove();
	});
	...
	for (const key of serverKeys) {
		...
		const sectionCls = `dyom-server-section-${safeKey}`;
		const bodyId = `server-body-${safeKey}`;
		let sec = _ensureSection(sectionCls, sg.name, allTasks);
		if (allTasks.length && !sec.querySelector('.dyom-section-header')) {
			...
			sec.insertAdjacentHTML('afterbegin', `<div class="dyom-section-header" data-collapse="${bodyId}"><svg class="dyom-section-chevron" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><polyline points="6 9 12 15 18 9"/></svg><span class="dyom-srv-status ${_secDot}" title="${_dotTitle}" style="flex-shrink:0;position:relative;top:0px;"></span><span class="dyom-section-title" style="margin:0;">${esc(sg.name)}</span><button class="dyom-btn dyom-stop-all-btn" data-stop-server="${esc(key)}">Stop all</button><button class="dyom-btn dyom-clear-btn" data-clear-server="${esc(key)}">Clear finished</button></div><div id="${bodyId}" class="dyom-section-body"></div>`);
		}
	}
	...
	group.querySelectorAll('[data-clear-server]').forEach(btn => {
		...
		btn.addEventListener('click', async (e) => {
			...
			toRemove.forEach(t => {
				const el = document.querySelector(`.dyom-task[data-task-id="${t.sessionId}"]`);
				if (el) {
					if (el._abort) el._abort.abort();
			...
			setTimeout(() => {
				toRemove.forEach(t => document.querySelector(`.dyom-task[data-task-id="${t.sessionId}"]`)?.remove());
				...
				const _sec = group.querySelector(`.dyom-server-section-${_sk}`);
				if (_sec && !_sec.querySelector('.dyom-task')) _sec.remove();
	...
	group.querySelectorAll('[data-stop-server]').forEach(btn => {
		...
		btn.addEventListener('click', async (e) => {
			...
			running.forEach(t => {
				const el = document.querySelector(`.dyom-task[data-task-id="${t.sessionId}"]`);
				el?.querySelector('.dyom-task-action-stop')?.click();
			});
	...
	group.querySelectorAll('.dyom-section-header[data-collapse]').forEach(hdr => {
		...
		hdr.addEventListener('click', () => {
			...
			const chevron = hdr.querySelector('.dyom-section-chevron');
			if (chevron) {
	...
	group.querySelectorAll('.dyom-task').forEach(el => {
		...
		if (task) {
			...
			const typeChip = el.querySelector('.dyom-task-type');
			if (typeChip) {
				...
				typeChip.classList.toggle('dyom-task-type-done', isDoneDl);
			}
			const badge = el.querySelector('.dyom-task-status');
			if (badge) {
				const _bdg = _taskBadge(task);
				badge.textContent = _bdg.text;
				badge.className = 'dyom-task-status' + (_bdg.cls ? ' ' + _bdg.cls : '');
			...
			const check = el.querySelector('.dyom-task-check');
	...
	for (const task of tasks) {
		...
		el.className = 'dyom-task' + (task._unreachable && task.status === 'running' ? ' dyom-task-unreachable' : '');
		...
		el.innerHTML = `
      <div class="dyom-task-header">
        <span class="dyom-task-type${(task.status === 'done' && task.type === 'download') ? ' dyom-task-type-done' : ''}" data-type="${esc(task.type)}">${esc((task.status === 'done' && task.type === 'download') ? 'finished' : task.type)}</span>
        <span class="dyom-task-name">${modelLogo(task.name)}${esc(task.name)}</span>
        <span class="dyom-task-status ${_bdg.cls}" style="display:${task.status === 'done' ? 'none' : ''}"${_bdgTitle}>${esc(_bdg.text)}</span>
        ${task.type === 'serve' && task.payload?._cmd ? '<button class="dyom-task-edit-btn" title="Edit settings &amp; relaunch"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>' : ''}
        ${task.type === 'serve' && task.payload?._cmd ? '<button class="dyom-task-save-btn" title="Save preset"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg></button>' : ''}
        <span class="dyom-task-indicator"><span class="dyom-task-wave" style="display:${task.status === 'running' ? '' : 'none'}"></span><span class="dyom-task-check" title="Clear" style="display:${(task.status === 'done' && !(task.type === 'download' && !task.payload?._dep)) ? '' : 'none'}"><svg class="dyom-task-check-ico" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#50fa7b" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg><svg class="dyom-task-clear-ico" width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg><span class="dyom-task-done-label">done</span><span class="dyom-task-clear-label">clear</span></span></span>
        ${task.type === 'download' && !task.payload?._dep && task.status === 'done' ? `<span class="dyom-task-status dyom-task-done">finished</span>` : ''}
        <button class="dyom-task-menu-btn" title="Actions">&#8942;</button>
      </div>
      <div class="dyom-task-sub"><span class="dyom-task-session">${esc(task.sessionId)}</span><span class="dyom-task-uptime" style="display:${((task.type === 'serve' || task.type === 'download') && task.status === 'running') ? '' : 'none'}"></span></div>
      <div class="dyom-output-wrap dyom-task-collapsible${_mobileCollapseDefault ? ' dyom-task-collapsed' : ''}"><pre class="dyom-output-pre">${esc(task.output || '')}</pre><button type="button" class="copy-code dyom-output-copy"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button></div>
    `;

		const _waveEl = el.querySelector('.dyom-task-wave');
		if (_waveEl && task.status === 'running') _registerWaveEl(_waveEl);

		const _uptimeEl = el.querySelector('.dyom-task-uptime');
		...
		const editBtn = el.querySelector('.dyom-task-edit-btn');
		if (editBtn) {
			editBtn.addEventListener('click', (e) => { e.stopPropagation(); _openEdit(); });
		}

		// Wire save icon button
		const saveBtn = el.querySelector('.dyom-task-save-btn');
		if (saveBtn) {
		...
		if (task.type === 'download') {
			const _serveBtn = el.querySelector('.dyom-task-serve-btn');
			if (_serveBtn) {
		...
		const _clearChk = el.querySelector('.dyom-task-check');
		if (_clearChk) {
		...
		el.querySelector('.dyom-task-header').addEventListener('click', (e) => {
			if (e.target.closest('button')) return;
			const wrap = el.querySelector('.dyom-output-wrap');
			if (wrap) wrap.classList.toggle('dyom-task-collapsed');
		});
		...
		const menuBtn = el.querySelector('.dyom-task-menu-btn');
		if (menuBtn) {
			...
			el.addEventListener('touchstart', (e) => {
				...
				if (e.target.closest('button, a, input, textarea, .dyom-task-dropdown')) return;
				_lpStart(e);
			}, { passive: true });
			...
			menuBtn.addEventListener('click', (e) => {
				e.stopPropagation();
				document.querySelectorAll('.dyom-task-dropdown').forEach(d => d.remove());

				const dropdown = document.createElement('div');
				dropdown.className = 'dyom-task-dropdown';
				...
				if (task.type === 'download' && task.status === 'queued') {
					items.push({
						label: 'Start now', action: 'start-now', custom: () => {
							_startQueuedDownload(task);
							_renderRunningTab();
						}
					});
				...
				if (task.type === 'serve' && task.payload?._cmd) {
					items.push({
						label: 'Save serve', action: 'save', custom: () => {
				...
				if (task.type === 'serve' && task.status !== 'running' && task.payload?._cmd) {
					items.push({
						label: 'Edit command', action: 'edit', custom: async () => {
				...
				if (task.type === 'serve' && task.payload?._cmd) {
					items.push({
						label: 'Register endpoint', action: 'register-endpoint', custom: async () => {
				...
				if (_isWindows(task)) {
					...
					items.push({
						label: 'Copy log cmd', action: 'copy-tmux', custom: () => {
							_copyText(logCmd);
						}
					});
				} else {
					...
					items.push({
						label: 'Copy tmux', action: 'copy-tmux', custom: () => {
				...
				items.push({
					label: 'Copy last 50 lines', action: 'copy-log', custom: () => {
						const out = (el.querySelector('.dyom-output-pre')?.textContent || task.output || '');
				...
				for (const item of items) {
					...
					div.className = 'dropdown-item-compact'
						+ (item.danger ? ' dyom-dropdown-danger' : '')
					...
					div.addEventListener('click', () => {
						...
						el.querySelector('.dyom-task-action-' + item.action)?.click();
					});
      ...
      <button class="dyom-task-action-reconnect"></button>
      <button class="dyom-task-action-retry"></button>
      <button class="dyom-task-action-stop"></button>
      <button class="dyom-task-action-kill"></button>
    `;
		el.appendChild(_actionBtns);

		// Wire reconnect
		el.querySelector('.dyom-task-action-reconnect').addEventListener('click', () => {
			_updateTask(task.sessionId, { status: 'running' });
			el.dataset.status = 'running';
			const badge = el.querySelector('.dyom-task-status');
			if (badge) { badge.textContent = _statusLabel('running', task.type); badge.className = 'dyom-task-status dyom-task-running'; }
			_reconnectTask(el, task);
		});

		// Wire stop
		el.querySelector('.dyom-task-action-stop').addEventListener('click', async () => {
			const badge = el.querySelector('.dyom-task-status');
			if (badge) { badge.textContent = 'stopping...'; badge.className = 'dyom-task-status dyom-task-stopping'; }
		...
		// Wire kill
		el.querySelector('.dyom-task-action-kill').addEventListener('click', () => {
		...
		// Wire retry
		el.querySelector('.dyom-task-action-retry').addEventListener('click', () => _retryTask(el, task));

		// Wire copy button
		el.querySelector('.dyom-output-copy').addEventListener('click', (e) => {
			e.stopPropagation();
			const text = el.querySelector('.dyom-output-pre')?.textContent || '';
			_copyText(text).then(() => {
				const btn = el.querySelector('.dyom-output-copy');
	...
	_collapsedTaskIds.forEach((id) => {
		const wrap = body.querySelector(`.dyom-task[data-task-id="${id}"] .dyom-output-wrap`);
		if (wrap) wrap.classList.add('dyom-task-collapsed');
	});
	...
	if (_mobileCollapseDefault) {
		_expandedTaskIds.forEach((id) => {
			const wrap = body.querySelector(`.dyom-task[data-task-id="${id}"] .dyom-output-wrap`);
			if (wrap) wrap.classList.remove('dyom-task-collapsed');
		});
	}
	_collapsedSectionIds.forEach((sid) => {
		const sb = document.getElementById(sid);
		if (sb) sb.style.display = 'none';
		const hdr = body.querySelector(`.dyom-section-header[data-collapse="${sid}"]`);
		const chevron = hdr?.querySelector('.dyom-section-chevron');
...
async function _reconnectTask(el, task) {
	const output = el.querySelector('.dyom-output-pre');
	...
	while (!controller.signal.aborted) {
		...
		try {
			...
			if (data.exit_code !== 0) {
				...
				if (diag) {
					let diagEl = el.querySelector('.dyom-diagnosis');
					if (!diagEl) {
						diagEl = document.createElement('div');
						diagEl.className = 'dyom-diagnosis';
					...
					const badge = el.querySelector('.dyom-task-status');
					if (badge) { badge.textContent = _statusLabel('error', task.type); badge.className = 'dyom-task-status dyom-task-error'; }
					_showDYOMNotif(true);
				} else {
					...
					if (!lastOutput.trim() || (task.type === 'download' && !looksSuccessful)) {
						...
						const badge = el.querySelector('.dyom-task-status');
						if (badge) { badge.textContent = _statusLabel('crashed', task.type); badge.className = 'dyom-task-status dyom-task-crashed'; }
						_showDYOMNotif(true);
					} else {
						_updateTask(task.sessionId, { status: 'done' });
						el.dataset.status = 'done';
						const badge = el.querySelector('.dyom-task-status');
						if (badge) { badge.textContent = _statusLabel('done', task.type); badge.className = 'dyom-task-status dyom-task-done'; }
						const _chk = el.querySelector('.dyom-task-check'); if (_chk && task.type !== 'download') _chk.style.display = '';
						const _sb = el.querySelector('.dyom-task-serve-btn'); if (_sb) _sb.style.display = '';
						_showDYOMNotif();
					}
				}
				_renderRunningTab();
			...
			if (snapshot) {
				...
				if (task.type === 'download') {
					...
					if (badge) {
						...
						if (curProgress !== el._lastProgress) {
							...
							badge.className = 'dyom-task-status dyom-task-error';
							...
							try {
								...
								if (data.ok && data.session_id) {
									...
									badge.className = 'dyom-task-status dyom-task-running';
									continue;
								}
							} catch { }
						...
						if (snapshot.includes('DOWNLOAD_FAILED')) {
							...
							if (_accessDenied) {
								...
								if (_diag) {
									let diagEl = el.querySelector('.dyom-diagnosis');
									if (!diagEl) { diagEl = document.createElement('div'); diagEl.className = 'dyom-diagnosis'; el.appendChild(diagEl); }
						...
						if (snapshot.includes('DOWNLOAD_OK') || (snapshot.includes('/snapshots/') && completed >= totalFiles && totalFiles > 0)) {
							...
							badge.className = 'dyom-task-status dyom-task-done';
							// Flip the type chip from "download" to the green "finished"
							// badge so the header reads as completed without a stale label.
							const _typeChip = el.querySelector('.dyom-task-type');
							if (_typeChip) { _typeChip.textContent = 'finished'; _typeChip.classList.add('dyom-task-type-done'); }
							_updateTask(task.sessionId, { status: 'done' });
							const _sb2 = el.querySelector('.dyom-task-serve-btn'); if (_sb2) _sb2.style.display = '';
							_showDYOMNotif();
							fetch('/api/shell/exec', {
								method: 'POST', credentials: 'same-origin',
								headers: { 'Content-Type': 'application/json' },
					...
					if (badge) {
						...
						if (info.phase) {
							...
							if (info.status === 'ready' && task._unreachable) {
								...
								el.classList.remove('dyom-task-unreachable');
								_refreshServerDots();
							}
				...
				if (task.type === 'serve' && !task._endpointAdded && !task._endpointAddInFlight && task._serveReady) {
					...
					_updateTask(task.sessionId, { status: 'running' });
					const badge = el.querySelector('.dyom-task-status');
					if (badge) { badge.textContent = 'running'; badge.className = 'dyom-task-status dyom-task-running'; }
					_showDYOMNotif();
				}
				// Detect process exit
				if (snapshot.includes('=== Process exited with code')) {
					...
					const badge = el.querySelector('.dyom-task-status');
					if (badge) { badge.textContent = status; badge.className = `dyom-task-status dyom-task-${status}`; }
					_renderRunningTab();
				}
...
async function _checkServeReachability() {
	...
	for (const task of serveTasks) {
		...
		const el = document.querySelector(`.dyom-task[data-task-id="${task.sessionId}"]`);
		if (el) {
			el.classList.toggle('dyom-task-unreachable', unreachable);
			const badge = el.querySelector('.dyom-task-status');
			if (badge) {
				if (unreachable) {
		...
		if (unreachable) _showDYOMNotif(true);
	}
	_refreshServerDots();
}
...
function _syncSettingsServerDots(byKey) {
	document.querySelectorAll('.dyom-server-entry').forEach(entry => {
		const hostEl = entry.querySelector('.dyom-srv-host');
		const dot = entry.querySelector('.dyom-srv-status');
		const msg = entry.querySelector('.dyom-srv-test-msg');
...
function _refreshServerDots() {
	...
	document.querySelectorAll('.dyom-section-header').forEach(header => {
		const dot = header.querySelector('.dyom-srv-status');
		if (!dot) return;
...
function _stopBackgroundMonitor() {
	...
	const statusEl = document.getElementById('dyom-bg-status');
	if (statusEl) statusEl.style.display = 'none';
}

// Retry-probe a freshly-added endpoint until its model server answers.
// A model that just reached "ready" in the dyom often can't satisfy
...
async function _pollBackgroundStatus() {
	try {
		...
		try {
			const stateRes = await fetch('/api/dyom/state', { credentials: 'same-origin' });
			if (stateRes.ok) {
		...
		const res = await fetch('/api/dyom/tasks/status', { credentials: 'same-origin' });
		if (!res.ok) return;
		const data = await res.json();
		...
		// Without this, the dyom icon stays at full opacity ("highlighted")
		// indefinitely once any task fires the notif, because the modal-open
		// clear only runs when the user actually reopens DYOM.
