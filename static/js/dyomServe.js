...
function _isActivelyDownloading(repoId) {
	try {
		const tasks = JSON.parse(localStorage.getItem("dyom-tasks")) || [];
...
function _rerenderCachedModels() {
	...
	for (const m of allModels) {
		...
		if (m.status === "downloading") {
			...
			metaParts.push(
				`<span class="dyom-dl-status" style="color:var(--accent,var(--red));">${_active ? "downloading" : "download stalled"}</span>`,
			);
		}
		...
		const _runningPill = _isActivelyServing(m.repo_id)
			? ' <span class="dyom-serve-running-pill" title="This model is currently being served">running</span>'
			: "";
		html += `<div class="memory-item-title"${_mc ? ` style="color:${_mc}"` : ""}>${modelLogo(m.repo_id)}${esc(shortName)}${hfLink ? ` <a href="${esc(hfLink)}" target="_blank" rel="noopener" class="dyom-hf-link">HF ↗</a>` : ""}${_runningPill}</div>`;
		...
		html += `<span class="dyom-card-backend" data-detected="${_bk}">${_bkIco}</span>`;
	...
	list.querySelectorAll(".hwfit-cached-menu-btn").forEach((btn) => {
		btn.addEventListener("click", (e) => {
			...
			if (existing && existing._anchor === btn) {
				existing.remove();
				btn.classList.remove("dyom-menu-active");
				return;
			}
			...
			document.querySelectorAll(".hwfit-cached-dropdown").forEach((d) => {
				if (d._anchor) d._anchor.classList.remove("dyom-menu-active");
				d.remove();
			});
			...
			btn.classList.add("dyom-menu-active");
	...
	list.querySelectorAll(".memory-item[data-repo]").forEach((item) => {
		item.addEventListener("click", (e) => {
			...
			const _selSrv =
				(_es.servers || []).find((s) => s.host === (_es.remoteHost || "")) ||
				{};
			...
			const _slotsHtml =
				`<div class="dyom-serve-slots dyom-saved-split">` +
				`<button type="button" class="dyom-slot-btn dyom-saved-save" title="Save current config"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11l5 5v11a2 2 0 0 1-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>Save</button>` +
				`<button type="button" class="dyom-slot-btn dyom-saved-arrow" title="Saved launch configs">${_arrowLabel}</button>` +
				`</div>`;
			...
			for (let i = 0; i < _gpuMax; i++) {
				...
				_gpuBtnsHtml += `<button type="button" class="dyom-gpu-btn${on ? " active" : ""}" data-gpu="${i}">${i}</button>`;
			}
			panelHtml += `<label>${_l("GPUs", "Toggle which GPUs to use")}<div class="dyom-gpu-group">${_gpuBtnsHtml}</div><input type="hidden" class="hwfit-sf" data-field="gpus" value="${esc(defaultGpus)}" /></label>`;
			panelHtml += `</div>`;
			// Row 2: Core settings
			...
			if (_opts2.flags.includes("--enable-expert-parallel"))
				panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="expert_parallel" /> Expert Parallel</label>`;
			if (_opts2.flags.some((f) => f.includes("--reasoning-parser"))) {
				const rp = _opts2.flags
					.find((f) => f.includes("--reasoning-parser"))
					.split(" ")[1];
				panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="reasoning_parser" data-parser="${rp}" /> Reasoning Parser <span class="hwfit-parser-tag">${rp}</span></label>`;
			}
			{
				...
				// adds the flag at launch (see dyom.js command builder).
			...
			if (_opts2.envVars.length)
				panelHtml += `<label class="hwfit-sf-cb"><input type="checkbox" class="hwfit-sf" data-field="moe_env" /> MoE Env Vars</label>`;
			panelHtml += `</div>`;
			...
			panelHtml += `<button type="button" class="dyom-btn hwfit-serve-copy hwfit-serve-copy-inline" title="Copy launch command" aria-label="Copy"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg></button>`;
			panelHtml += `</div>`;
			panelHtml += `<div class="hwfit-serve-actions">`;
			// Split button: main "Clear Server" + caret that opens Probe / Cancel.
			// The .dyom-gpu-probe button stays in the DOM but hidden so the
			// existing event-listener wiring further down keeps working — the
			// popup just programmatically clicks it.
			panelHtml += `<span class="dyom-gpu-split">`;
			panelHtml += `<button class="dyom-btn dyom-gpu-clear dyom-gpu-split-main" title="Clear server GPU memory by stopping processes that hold VRAM (SIGTERM first)">Clear Server</button>`;
			panelHtml += `<button class="dyom-btn dyom-gpu-split-arrow" type="button" aria-haspopup="menu" aria-label="More GPU actions" title="More GPU actions"><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 15 12 9 18 15"/></svg></button>`;
			panelHtml += `</span>`;
			panelHtml += `<button class="dyom-btn dyom-gpu-probe" style="display:none;" title="Probe GPU memory and running GPU processes">Probe GPUs</button>`;
			...
			panelHtml += `<button class="dyom-btn hwfit-serve-cancel" type="button" title="Close this configuration panel">Cancel</button>`;
			panelHtml += `<button class="dyom-btn hwfit-serve-launch">Launch</button>`;
			panelHtml += `</div>`;
			panelHtml += `</div>`;
			...
			function _loadSlotIntoPanel(slotIdx) {
				...
				panel.querySelectorAll(".dyom-gpu-btn").forEach((btn) => {
				...
				updateCmd();
				panel
					.querySelectorAll(".dyom-slot-btn")
					.forEach((b) => b.classList.remove("active"));
				panel
					.querySelector(`.dyom-slot-btn[data-slot="${slotIdx}"]`)
			...
			function _updateSavedToggleLabel() {
				...
				const t = panel.querySelector(".dyom-saved-arrow");
			...
			// reflects the stored presets. Standard digital_garage .dropdown look, positioned
			// fixed at the toggle and right-aligned to it.
			function _showSavedConfigMenu(anchor) {
				document
					.querySelectorAll(".dyom-saved-menu")
					.forEach((d) => d.remove());
				const modelSlots = _presetsForModel(_loadPresets(), repo);
				const dropdown = document.createElement("div");
				dropdown.className = "dropdown dyom-saved-menu";
				...
				modelSlots.forEach((p, idx) => {
					...
					if (p.confirmedWorking) {
						...
						badge.className = "dyom-saved-confirmed";
						badge.title =
					...
					it.addEventListener("click", (e) => {
						...
						if (_cmdBox) {
							_cmdBox.classList.add("dyom-cmd-flash");
							setTimeout(
								() => _cmdBox.classList.remove("dyom-cmd-flash"),
								600,
							);
						}
					});
				...
				const close = (ev) => {
					if (
						...
						dropdown.remove();
						anchor.classList.remove("dyom-menu-active");
						document.removeEventListener("click", close, true);
					}
				};
			...
			const savedSaveBtn = panel.querySelector(".dyom-saved-save");
			if (savedSaveBtn) {
			...
			const savedArrowBtn = panel.querySelector(".dyom-saved-arrow");
			if (savedArrowBtn) {
				savedArrowBtn.addEventListener("click", (e) => {
					e.stopPropagation();
					if (document.querySelector(".dyom-saved-menu")) {
						document
							.querySelectorAll(".dyom-saved-menu")
							.forEach((d) => d.remove());
						savedArrowBtn.classList.remove("dyom-menu-active");
						return;
					}
					savedArrowBtn.classList.add("dyom-menu-active");
			...
			panel.querySelectorAll(".dyom-gpu-btn").forEach((btn) => {
				btn.addEventListener("click", () => {
					...
					const activeBtns = [
						...panel.querySelectorAll(".dyom-gpu-btn.active"),
					];
			...
			const _probeBtn = panel.querySelector(".dyom-gpu-probe");
			const _clearBtn = panel.querySelector(".dyom-gpu-clear");
			const _splitArrow = panel.querySelector(".dyom-gpu-split-arrow");
			...
			// logic by programmatically clicking the hidden .dyom-gpu-probe.
			if (_splitArrow) {
				_splitArrow.addEventListener("click", (ev) => {
					ev.stopPropagation();
					document
						.querySelectorAll(".dyom-gpu-split-menu")
						.forEach((m) => m.remove());
					const menu = document.createElement("div");
					menu.className = "dyom-task-dropdown dyom-gpu-split-menu";
			...
			if (_probeBtn) {
				...
				const _doKill = async (pid, sig, hostVal) => {
					const res = await fetch("/api/dyom/kill-pid", {
						method: "POST",
				...
				const _openProbePopup = (anchorBtn, gpu, hostVal) => {
					...
					popup.className = "dyom-gpu-popup";
					const procs = gpu.processes || [];
					const procHtml =
						procs.length === 0
							? '<div class="dyom-gpu-popup-empty">No GPU processes reported. VRAM may be held by a zombie or another tenant.</div>'
							: procs
									.map(
										(p) =>
											`<div class="dyom-gpu-proc" data-pid="${p.pid}">
                   <span class="dyom-gpu-proc-info">
                     <span class="dyom-gpu-proc-pid">${p.pid}</span>
                     <span class="dyom-gpu-proc-name" title="${esc(p.name)}">${esc(p.name)}</span>
                     <span class="dyom-gpu-proc-mem">${(p.used_mb / 1024).toFixed(1)}G</span>
                   </span>
                   <span class="dyom-gpu-proc-actions">
                     <button type="button" class="dyom-gpu-kill" data-sig="TERM" title="Graceful (SIGTERM)">Kill</button>
                     <button type="button" class="dyom-gpu-kill" data-sig="KILL" title="Force (SIGKILL)">!</button>
                   </span>
                 </div>`,
									)
									.join("");
					popup.innerHTML = `
            <div class="dyom-gpu-popup-head">
              GPU ${gpu.index} · ${esc(gpu.name)}
              <span class="dyom-gpu-popup-stats">${(gpu.free_mb / 1024).toFixed(1)} / ${(gpu.total_mb / 1024).toFixed(1)} GB free · util ${gpu.util_pct}%</span>
              <button type="button" class="dyom-gpu-popup-close" title="Close">×</button>
            </div>
            <div class="dyom-gpu-popup-body">${procHtml}</div>`;
					...
					popup
						.querySelector(".dyom-gpu-popup-close")
						?.addEventListener("click", _closeProbePopup);
					popup.querySelectorAll(".dyom-gpu-kill").forEach((btn) => {
						btn.addEventListener("click", async (ev) => {
							ev.stopPropagation();
							const row = btn.closest(".dyom-gpu-proc");
				...
				const _runProbe = async (silent = false) => {
					...
					panel.querySelectorAll(".dyom-gpu-btn").forEach((b) => {
			...
			panel
				...
				.addEventListener("click", async (ev) => {
					...
					const launchCmd = _cmdTextarea
						? _cmdTextarea.value.trim()
						: panel._cmd;
					const serveState = {};
					panel.querySelectorAll(".hwfit-sf").forEach((el) => {
						if (el.type === "checkbox")
					...
					serveState.backend =
						_detectBackend(m).backend || serveState.backend || "vllm";
					...
					try {
						...
						localStorage.setItem(
							SERVE_STATE_KEY,
							JSON.stringify({ _byRepo: byRepo, _lastUsed: serveState }),
						);
					} catch {}
					...
					if (_ssEl && _ssEl.value != null) {
						if (_ssEl.value === "local") serveHost = "";
							...
							const _srv =
								_envState.servers.find((s) => s.host === _ssEl.value) ||
								_envState.servers[parseInt(_ssEl.value)];
							if (_srv) {
								serveHost = _srv.host;
								_srvEnv = _srv.env || "";
...
function _resolveCacheHost() {
	...
	if (cacheSrv) {
		...
		if (val === "local") host = "";
			...
			const s =
				_envState.servers.find((x) => x.host === val) ||
				_envState.servers[parseInt(val)];
			if (s) host = s.host;
		}
	}
	return host;
...
async function _deleteCachedModel(
	...
	if (itemEl) {
		...
		ov.className = "dyom-delete-overlay";
	...
	try {
		...
		if (itemEl) {
			itemEl.querySelector(".dyom-delete-overlay")?.remove();
...
function _retryCachedModel(repo, m) {
	...
	if (_envState.remoteHost) {
		payload.remote_host = _envState.remoteHost;
		const _sp2 = _getPort(_envState.remoteHost);
		if (_sp2) payload.ssh_port = _sp2;
	}
...
export async function _fetchCachedModels() {
	...
	try {
		...
		const _dirsEl = document.querySelector(".dyom-serve-dirs");
		if (_dirsEl && selectedServer) {
			...
			_dirsEl.innerHTML =
				_allDirs
					.map((d) => `<span class="dyom-serve-dir-pill">${esc(d)}</span>`)
					.join("") +
				'<span class="dyom-serve-dir-edit" title="Edit in Settings">edit</span>';
			_dirsEl
				.querySelector(".dyom-serve-dir-edit")
				?.addEventListener("click", () => {
					document
						.querySelector('#dyom-modal .dyom-tab[data-backend="Settings"]')
						?.click();
				});
		}
		...
		const ready = data.models.filter(
			(m) => m.status === "ready" && !m.size.includes("MB"),
		);
