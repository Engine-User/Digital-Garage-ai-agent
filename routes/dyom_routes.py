# Digital Garage@2026
"""DYOM routes — model download, serve, cache scanning, and dyom state sync."""
...
from routes.dyom_helpers import (
...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[Digital_Garage] HF token: applied"; '
    'else '
    'echo "[Digital_Garage] HF token: NOT SET — gated/private models will be denied. '
    'Add one in Digital_Garage Settings -> DYOM -> HuggingFace Token."; '
    'fi'
)

def setup_dyom_routes() -> APIRouter:
    router = APIRouter(tags=["dyom"])
    _dyom_state_path = Path(os.environ.get("DATA_DIR", "data")) / "dyom_state.json"
    ...
    def _diagnose_serve_output(text: str) -> dict | None:
        """Server-side mirror of the DYOM UI's common serve diagnoses.

        The browser uses dyom-diagnosis.js for clickable fixes. This gives
        ...
        patterns = [
            ...
            (
                ...
                [{"label": "clear DYOM GPU selection or choose available GPUs", "op": "settings", "field": "gpus", "value": ""}],
            ),
            (
                r"vllm.*command not found|No module named vllm|ERROR: vLLM is not installed",
                "vLLM is not installed or not in PATH on this server.",
                [{"label": "install vLLM in DYOM Dependencies", "op": "dependency", "package": "vllm"}],
            ),
            (
                r"sglang.*command not found|No module named sglang|SGLang is not installed",
                "SGLang is not installed or not in PATH on this server.",
                [{"label": "install SGLang in DYOM Dependencies", "op": "dependency", "package": "sglang[all]"}],
            ),
            (
    ...
    def _state_for_client(state):
        """Return dyom state without raw secrets for browser clients."""
    ...
    def _state_for_storage(state, on_disk=None):
        """Encrypt dyom secrets before writing state to disk."""
        _strip_task_secrets(state)
        env = state.get("env") if isinstance(state, dict) else None
    ...
    def _load_stored_hf_token() -> str:
        if not _dyom_state_path.exists():
            return ""
        try:
            state = json.loads(_dyom_state_path.read_text())
            env = state.get("env") if isinstance(state, dict) else {}
            return _decrypt_secret(env.get("hfToken") if isinstance(env, dict) else "")
        except Exception:
            return ""

    def _dyom_ssh_dir() -> Path:
        app_ssh = Path("/app/.ssh")
        if Path("/app").exists():
            return app_ssh
        return Path.home() / ".ssh"

    def _dyom_ssh_key_path() -> Path:
        return _dyom_ssh_dir() / "id_ed25519"

    def _read_dyom_public_key() -> str:
        pub = _dyom_ssh_key_path().with_suffix(".pub")
        if not pub.exists():
            return ""
        return pub.read_text(encoding="utf-8", errors="replace").strip()

    @router.get("/api/dyom/ssh-key")
    async def get_dyom_ssh_key(request: Request):
        require_admin(request)
        public_key = _read_dyom_public_key()
        return {
            "configured": bool(public_key),
            "public_key": public_key,
        }

    @router.post("/api/dyom/ssh-key")
    async def generate_dyom_ssh_key(request: Request):
        require_admin(request)
        ssh_dir = _dyom_ssh_dir()
        key_path = _dyom_ssh_key_path()
        ...
        if not key_path.exists():
            proc = await asyncio.create_subprocess_exec(
                "ssh-keygen", "-t", "ed25519", "-N", "", "-C", "Digital_Garage-dyom", "-f", str(key_path),
        ...
        try:
            ...
            pass
        return {"ok": True, "public_key": _read_dyom_public_key()}

    def _user_shell_path_bootstrap() -> list[str]:
        return [
            'DIGITAL_GARAGE_USER_SHELL="${SHELL:-}"',
            'if [ -n "$DIGITAL_GARAGE_USER_SHELL" ] && [ -x "$DIGITAL_GARAGE_USER_SHELL" ]; then',
            '  DIGITAL_GARAGES_USER_PATH="$("$DIGITAL_GARAGE_USER_SHELL" -ic \'printf "__DIGITAL_GARAGE_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__DIGITAL_GARAGE_PATH__//p\' | tail -n 1 || true)"',
            '  if [ -n "$DIGITAL_GARAGE_USER_PATH" ]; then export PATH="$DIGITAL_GARAGE_USER_PATH:$PATH"; fi',
            'fi',
        ]
    ...
    def _missing_binary_message(binary: str, target: str) -> str:
        if binary == "tmux":
            return (
                f"tmux is required for DYOM background downloads/serves on {target}. "
                "Install it with your OS package manager, or run DYOM server setup for that server."
            )
        if binary == "docker":
            return (
                f"Docker is required by this DYOM launch command on {target}, but the docker CLI was not found. "
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        session_id = f"dyom-{uuid.uuid4().hex[:8]}"
        ...
        if remote and is_windows:
            ...
            ps_lines = []
            ps_lines.append('$sessionDir = "$env:TEMP\\Digital_Garage_sessions"')
            ...
            launch_ps = (
                "$sd = \\\"$env:TEMP\\Digital_Garage_sessions\\\"; "
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('  echo "ERROR: vLLM is not installed. Open DYOM -> Dependencies and install vllm on this server, then launch again."')
                runner_lines.append('  exit 127')
                runner_lines.append('fi')
            elif "sglang.launch_server" in req.cmd:
                runner_lines.append('export PATH="$HOME/.local/bin:$PATH"')
                runner_lines.append('if ! python3 -c "import sglang" 2>/dev/null; then')
                runner_lines.append('  echo "ERROR: SGLang is not installed. Open DYOM -> Dependencies and install sglang on this server, then launch again."')
                runner_lines.append('  exit 127')
                runner_lines.append('fi')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if platform == "windows":
            ...
            setup_script = (
                'powershell -Command "'
                "New-Item -ItemType Directory -Force -Path $env:TEMP\\Digital_Garage_sessions | Out-Null; "
    ...
    # ── DYOM state persistence (cross-device sync) ──

    @router.get("/api/dyom/state")
    async def get_dyom_state(request: Request):
        """Load saved dyom state (tasks, servers, presets, settings)."""
        require_admin(request)
        if _dyom_state_path.exists():
            try:
                return _state_for_client(json.loads(_dyom_state_path.read_text()))
            except Exception:
                return {}
        return {}

    @router.post("/api/dyom/state")
    async def save_dyom_state(request: Request):
        """Save dyom state for cross-device sync.

        Admin-gated because dyom state is read back into shell-quoting
        ...
        try:
            ...
            try:
                if _dyom_state_path.exists():
                    on_disk = json.loads(_dyom_state_path.read_text())
                else:
                    on_disk = {}
            except Exception:
                on_disk = {}
            ...
            if disk_env:
                ...
                if inc_env is None:
                    data["env"] = disk_env
                    logger.warning("dyom state POST: incoming body had no env; preserved on-disk env (anti-wipe guard)")
                elif disk_env.get("servers") and not inc_env.get("servers"):
                    inc_env["servers"] = disk_env["servers"]
                    logger.warning("dyom state POST: incoming env.servers empty; preserved on-disk servers (anti-wipe guard)")
            ...
            for t in disk_tasks:
                ...
                if isinstance(ts, (int, float)) and (now_ms - ts) <= RACE_WINDOW_MS:
                    preserved.append(t)
            if preserved:
                logger.info(f"dyom state POST: preserving {len(preserved)} recent task(s) "
                            f"not in incoming body (race guard): "
                            f"{[t.get('sessionId') for t in preserved]}")
                data["tasks"] = incoming_tasks + preserved
            atomic_write_json(str(_dyom_state_path), _state_for_storage(data, on_disk), indent=2)
            return {"ok": True, "preserved": len(preserved)}
        except Exception as e:
            return {"ok": False, "error": str(e)}

    @router.get("/api/dyom/hf-latest")
    ...
    @router.get("/api/dyom/tasks/status")
    async def dyom_tasks_status(request: Request):
        """Check status of all active dyom tmux sessions.
        ...
        require_admin(request)
        return await asyncio.to_thread(_dyom_tasks_status_sync)

    def _dyom_tasks_status_sync():
        import subprocess

        # Load saved tasks from dyom state
        tasks = []
        if _dyom_state_path.exists():
            try:
                state = json.loads(_dyom_state_path.read_text())
        ...
        for task in tasks:
            ...
            # Defense-in-depth: dyom state is admin-writable but the values
