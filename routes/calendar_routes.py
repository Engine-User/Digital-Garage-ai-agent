...
import os as _os
FALLBACK_OWNER = _os.environ.get("DIGITAL_GARAGE_FALLBACK_OWNER", "owner@localhost")
_SINGLE_USER_MODE = _os.environ.get("DIGITAL_GARAGE_SINGLE_USER", "1") != "0"
    ...
    async def export_ics(request: Request, cal_id: str):
        ...
        try:
            ...
            lines = [
                "BEGIN:VCALENDAR",
                "VERSION:2.0",
                "PRODID:-//DigitalGarage//Calendar//EN",
            ...
            for ev in events:
                ...
                if ev.description:
                    newline_escape = '\\n'
                    lines.append(f"DESCRIPTION:{ev.description.replace(chr(10), newline_escape)}")
                if ev.location:
