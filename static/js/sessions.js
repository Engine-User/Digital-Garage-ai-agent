...
let _sortMode = Storage.get('digital_garage-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Garage Chat';
...
const FOLDER_STATE_KEY = 'digital_garage-folder-state';
const FOLDER_ORDER_KEY = 'digital_garage-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to digital_garage kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'digital_garage Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('digital_garage-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('digital_garage-session-sort', mode);
  else Storage.remove('digital_garage-session-sort');
  renderSessionList();
}
